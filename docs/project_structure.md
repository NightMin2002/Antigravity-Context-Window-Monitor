...
| 错误持久化 / Error persistence | 分账号隔离：`_persistedRetryErrorCodesByAccount`（email → 错误码计数）+ `_persistedRecentErrorsByAccount`（email → 消息列表）+ `_persistedUniqueErrorsByAccount`（email → 规范化消息 → `{ message, firstSeen }`，key 为 `normalizeErrorMessage(msg)` 而非 errorCode）存入 `state-v1.json`。切换账号时各账号数据独立保存不丢失，切回时恢复。errorCodes 使用 max-wins 合并（按账号桶隔离）。`baselineForQuotaReset()` 清除被归档账号的持久化错误数据，防止 max-wins 合并恢复已归档计数。旧版全局字段自动迁移至当前账号桶。旧版 errorCode-keyed 条目自动通过 `normalizeErrorMessage()` 重新分桶迁移。午夜 `reset()` 清空 |
