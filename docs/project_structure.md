...
面板总框架：标签切换（Monitor / Profile / Activity / GM Data / Pricing / Calendar / Quota Tracking / Settings）、消息通信。各标签内容由独立模块生成。

Panel framework: tab switching (Monitor / Profile / Activity / GM Data / Pricing / Calendar / Quota Tracking / Settings), message communication. Each tab's content is generated by independent modules.
...
|---|---|---|
| Summary Bar | `buildGMSummaryBar` | 调用数、步骤数、模型数、token 合计、积分 |
| Model Cards | `buildGMModelCards` | 每模型的调用数、TTFT、流速、token 明细 |
| Performance | `buildPerformanceChart` | TTFT avg/min/max、流速统计 |
| Cache | `buildCacheEfficiency` | 缓存倍率可视化条形图 |
| Context | `buildContextGrowth` | 上下文 token 增长趋势 SVG 折线图 |
| Conversations | `buildConversations` | 各对话的调用数和覆盖率 |
...
共 73 个测试，使用 `__mocks__/vscode.ts` 模拟 VS Code API。

73 total tests, using `__mocks__/vscode.ts` to mock the VS Code API.
