...
exports.buildCalendarTabContent = buildCalendarTabContent;
exports.getCalendarTabStyles = getCalendarTabStyles;
...
const fmtTok = (n) => n >= 1_000_000 ? (n / 1_000_000).toFixed(1) + 'M' : n >= 1000 ? (n / 1000).toFixed(1) + 'k' : String(n);
const fmtCostShort = (n) => n >= 1 ? '$' + n.toFixed(2) : n > 0 ? '$' + n.toFixed(4) : '';
...
function buildCalendarTabContent(store, year, month) {
    if (!store) {
        return `
            <section class="card empty">
                <h2>${CALENDAR_ICON} ${(0, i18n_1.tBi)('Calendar', '日历')}</h2>
                <p class="empty-desc">${(0, i18n_1.tBi)('Calendar data is not initialized yet.', '日历数据尚未初始化。')}</p>
            </section>`;
    }
    const now = new Date();
        ...
        parts.push(`
            <section class="card">
                <div class="cal-summary-header">
                    <h2>${webview_icons_1.ICON.chart} ${(0, i18n_1.tBi)('Usage Summary', '用量汇总')}</h2>
                    <div class="cal-summary-toggle" id="calSummaryToggle">
                        <button class="cal-summary-btn active" data-summary-mode="monthly">${(0, i18n_1.tBi)('Monthly', '月度')}</button>
                        <button class="cal-summary-btn" data-summary-mode="alltime">${(0, i18n_1.tBi)('All-Time', '全部')}</button>
                    </div>
                </div>
        ...
        parts.push(`
            <section class="card cal-clear-section">
                <button class="cal-clear-btn" id="clearCalendarBtn">
                    ${webview_icons_1.ICON.trash} ${(0, i18n_1.tBi)('Clear All Calendar History', '清空所有日历历史')}
                </button>
            </section>`);
    }
    return parts.join('');
...
// ─── Builders ────────────────────────────────────────────────────────────────
function buildMonthView(store, year, month) {
    ...
    const monthLabel = (0, i18n_1.tBi)(`${MONTH_NAMES_EN[month - 1]} ${year}`, `${year}年${MONTH_NAMES_ZH[month - 1]}`);
    const lang = (0, i18n_1.getLanguage)();
    ...
    for (let day = 1; day <= daysInMonth; day++) {
        ...
        if (hasData) {
            ...
            cells.push(`
                <button class="cal-cell has-data${todayClass}" data-cal-date="${dateStr}" data-tooltip="${cellData.cycleCount} ${(0, i18n_1.tBi)('cycles', '周期')}">
                    ${day}
...
function buildDayDetail(record, dateStr) {
    ...
    for (const c of record.cycles) {
        ...
        if (c.gmModelStats) {
            for (const [name, gm] of Object.entries(c.gmModelStats)) {
                ...
                if (gm.avgTTFT > 0 && gm.calls > 0) {
                    g.ttftSum += gm.avgTTFT * gm.calls;
                    g.ttftWeight += gm.calls;
                }
                if (gm.calls > 0) {
                    g.cacheSum += gm.cacheHitRate * gm.calls;
                    g.cacheWeight += gm.calls;
                }
    ...
    const todayBadge = isToday(dateStr)
        ? `<span class="badge info-badge">${(0, i18n_1.tBi)('TODAY', '今天')}</span>`
        : '';
    const cycleCount = record.cycles.length;
    const cycleSection = cycleCount > 1
        ? `<details class="cal-cycles-details" id="d-cal-cycles-${dateStr}"><summary class="cal-cycles-summary">${(0, i18n_1.tBi)(`View ${cycleCount} Individual Cycles`, `查看 ${cycleCount} 个独立周期`)}</summary>${cycleCards}</details>`
        : cycleCards;
    return `
        <div class="cal-detail" id="cal-detail-${dateStr}" style="display:none" data-cal-detail="${dateStr}">
            <div class="cal-detail-header">
                ...
                <div class="cal-detail-summary">
                    <span class="cal-sum-item"><span class="cal-stat-val">${cycleCount}</span> ${(0, i18n_1.tBi)('cycles', '周期')}</span>
                </div>
            </div>
            <div class="cal-day-summary">
                <div class="cal-day-total">
                    <div class="cal-day-total-val">${totalReasoning}</div>
                    <div class="cal-day-total-label">${(0, i18n_1.tBi)('Reasoning', '推理')}</div>
                </div>
                <div class="cal-day-total">
                    <div class="cal-day-total-val">${totalToolCalls}</div>
                    <div class="cal-day-total-label">${(0, i18n_1.tBi)('Tools', '工具')}</div>
                </div>
                <div class="cal-day-total">
                    <div class="cal-day-total-val">${formatTokensK(displayTokens)}</div>
                    <div class="cal-day-total-label">${(0, i18n_1.tBi)('Tokens', '令牌')}</div>
                </div>
                ${totalCost > 0 ? `
                <div class="cal-day-total">
                    <div class="cal-day-total-val">${formatCost(totalCost)}</div>
                    <div class="cal-day-total-label">${(0, i18n_1.tBi)('Cost', '费用')}</div>
                </div>` : ''}
                ${totalGMCredits > 0 ? `
                <div class="cal-day-total">
                    <div class="cal-day-total-val">${totalGMCredits}</div>
                    <div class="cal-day-total-label">${(0, i18n_1.tBi)('Credits', '积分')}</div>
                </div>` : ''}
                ${totalGMCalls > 0 ? `
                <div class="cal-day-total">
                    <div class="cal-day-total-val">${totalGMCalls}</div>
                    <div class="cal-day-total-label">${(0, i18n_1.tBi)('GM Calls', 'GM 调用')}</div>
                </div>` : ''}
                ${avgCacheRate > 0 ? `
                <div class="cal-day-total">
                    <div class="cal-day-total-val">${(avgCacheRate * 100).toFixed(0)}%</div>
                    <div class="cal-day-total-label">${(0, i18n_1.tBi)('Cache', '缓存')}</div>
                </div>` : ''}
                ${totalErrors > 0 ? `
                <div class="cal-day-total">
                    <div class="cal-day-total-val cal-day-total-danger">${totalErrors}</div>
                    <div class="cal-day-total-label">${(0, i18n_1.tBi)('Errors', '错误')}</div>
                </div>` : ''}
            </div>
            ${mergedModelHtml}
            ${mergedGMHtml}
...
function buildMergedModelRows(merged) {
    const entries = Object.entries(merged);
    if (entries.length === 0) {
        return '';
    }
    let html = '<div class="cal-model-rows">';
    html += `<div class="cal-gm-section-label">${(0, i18n_1.tBi)('Model Summary', '模型汇总')}</div>`;
    for (const [name, ms] of entries) {
        ...
        html += `<div class="cal-model-row"><span class="cal-model-name">${(0, webview_helpers_1.esc)(name)}</span><span class="cal-model-chips">${chips.join('')}</span></div>`;
    }
    html += '</div>';
    return html;
}
/** Merged per-model GM rows across all cycles (with weighted averages) */
function buildMergedGMRows(merged) {
    const entries = Object.entries(merged);
    if (entries.length === 0) {
        return '';
    }
    let html = '<div class="cal-model-rows">';
    html += `<div class="cal-gm-section-label">GM ${(0, i18n_1.tBi)('Summary', '汇总')}</div>`;
    for (const [name, ms] of entries) {
        const chips = [];
        if (ms.calls > 0) {
            chips.push(`<span class="cal-chip cal-chip-tools">${CAL_ICON.calls} ${ms.calls} ${(0, i18n_1.tBi)('calls', '调用')}</span>`);
        }
        if (ms.credits > 0) {
        ...
        if (avgTTFT > 0) {
            chips.push(`<span class="cal-chip cal-chip-ttft">${CAL_ICON.clock} ${avgTTFT.toFixed(1)}s</span>`);
        }
        const avgCache = ms.cacheWeight > 0 ? ms.cacheSum / ms.cacheWeight : 0;
        if (avgCache > 0) {
            chips.push(`<span class="cal-chip cal-chip-cache">${CAL_ICON.cache} ${(avgCache * 100).toFixed(0)}%</span>`);
        }
        if (ms.cost > 0) {
            chips.push(`<span class="cal-chip cal-chip-cost">${CAL_ICON.dollar} ${fmtCostShort(ms.cost)}</span>`);
        }
        const totalTok = ms.inputTokens + ms.outputTokens;
        if (totalTok > 0) {
            chips.push(`<span class="cal-chip cal-chip-tokens">${fmtTok(totalTok)} ${(0, i18n_1.tBi)('tok', '令牌')}</span>`);
        }
        html += `<div class="cal-model-row"><span class="cal-model-name">${(0, webview_helpers_1.esc)(name)}</span><span class="cal-model-chips">${chips.join('')}</span></div>`;
    }
    html += '</div>';
...
function buildCycleCard(cycle, index) {
    const startTime = (0, webview_helpers_1.formatShortTime)(cycle.startTime);
    const endTime = (0, webview_helpers_1.formatShortTime)(cycle.endTime);
    const duration = (0, webview_helpers_1.formatDuration)(new Date(cycle.endTime).getTime() - new Date(cycle.startTime).getTime());
    const modelChips = cycle.modelNames
        .map(m => `<span class="cal-model-chip">${(0, webview_helpers_1.esc)(m)}</span>`)
        .join('');
    const stats = [];
    if (cycle.totalReasoning > 0) {
        stats.push(`<span class="cal-stat"><span class="cal-stat-val">${cycle.totalReasoning}</span> <span class="cal-stat-label">${(0, i18n_1.tBi)('reasoning', '推理')}</span></span>`);
    }
    if (cycle.totalToolCalls > 0) {
        stats.push(`<span class="cal-stat"><span class="cal-stat-val">${cycle.totalToolCalls}</span> <span class="cal-stat-label">${(0, i18n_1.tBi)('tools', '工具')}</span></span>`);
    }
    if (cycle.totalInputTokens + cycle.totalOutputTokens > 0) {
        stats.push(`<span class="cal-stat"><span class="cal-stat-val">${formatTokensK(cycle.totalInputTokens + cycle.totalOutputTokens)}</span> <span class="cal-stat-label">${(0, i18n_1.tBi)('tokens', '令牌')}</span></span>`);
    }
    if (cycle.estimatedCost && cycle.estimatedCost > 0) {
        stats.push(`<span class="cal-stat"><span class="cal-stat-val">${formatCost(cycle.estimatedCost)}</span> <span class="cal-stat-label">${(0, i18n_1.tBi)('cost', '费用')}</span></span>`);
    }
    if (cycle.gmTotalCredits && cycle.gmTotalCredits > 0) {
        stats.push(`<span class="cal-stat"><span class="cal-stat-val">${cycle.gmTotalCredits}</span> <span class="cal-stat-label">${(0, i18n_1.tBi)('credits', '积分')}</span></span>`);
    }
    if (cycle.totalErrors > 0) {
        stats.push(`<span class="cal-stat cal-day-total-danger"><span class="cal-stat-val">${cycle.totalErrors}</span> <span class="cal-stat-label">${(0, i18n_1.tBi)('errors', '错误')}</span></span>`);
    }
    return `
        <div class="cal-cycle">
...
function buildPerModelRows(modelStats) {
    ...
    for (const [name, ms] of Object.entries(modelStats)) {
        ...
        html += `
            <div class="cal-model-row">
                <span class="cal-model-name">${(0, webview_helpers_1.esc)(name)}</span>
...
function buildGMModelRows(gmModelStats) {
    if (!gmModelStats || Object.keys(gmModelStats).length === 0) {
        return '';
    }
    let html = '<div class="cal-model-rows">';
    html += `<div class="cal-gm-section-label">GM ${(0, i18n_1.tBi)('Breakdown', '明细')}</div>`;
...
function buildOverallSummaryGrid(store) {
...
function buildMonthlySummaryGrid(store, year, month) {
    ...
    if (dates.length === 0) {
        ...
        return `<p class="cal-summary-empty">${(0, i18n_1.tBi)(`No data for ${monthLabel} yet.`, `${monthLabel}暂无数据。`)}</p>`;
    }
    let totalReasoning = 0, totalToolCalls = 0, totalCost = 0, totalCycles = 0;
    let totalErrors = 0, totalGMCalls = 0, totalGMCredits = 0, totalGMTokens = 0;
    for (const date of dates) {
...
function buildSummaryOverviewGrid(dayCount, cycleCount, reasoning, toolCalls, gmTokens, cost, gmCredits, gmCalls, errors) {
    return `
            <div class="cal-overview-grid">
                <div class="cal-overview-item">
                    <div class="cal-overview-val">${dayCount}</div>
                    <div class="cal-overview-label">${(0, i18n_1.tBi)('Days', '天数')}</div>
                </div>
                <div class="cal-overview-item">
                    <div class="cal-overview-val">${cycleCount}</div>
                    <div class="cal-overview-label">${(0, i18n_1.tBi)('Cycles', '周期')}</div>
                </div>
                <div class="cal-overview-item">
                    <div class="cal-overview-val">${reasoning}</div>
                    <div class="cal-overview-label">${(0, i18n_1.tBi)('Reasoning', '推理')}</div>
                </div>
                <div class="cal-overview-item">
                    <div class="cal-overview-val">${toolCalls}</div>
                    <div class="cal-overview-label">${(0, i18n_1.tBi)('Tools', '工具')}</div>
                </div>
                ${gmTokens > 0 ? `
                <div class="cal-overview-item">
                    <div class="cal-overview-val">${formatTokensK(gmTokens)}</div>
                    <div class="cal-overview-label">${(0, i18n_1.tBi)('Tokens', '令牌')}</div>
                </div>` : ''}
                ${cost > 0 ? `
                <div class="cal-overview-item">
                    <div class="cal-overview-val">${formatCost(cost)}</div>
                    <div class="cal-overview-label">${(0, i18n_1.tBi)('Cost', '费用')}</div>
                </div>` : ''}
                ${gmCredits > 0 ? `
                <div class="cal-overview-item">
                    <div class="cal-overview-val">${gmCredits}</div>
                    <div class="cal-overview-label">${(0, i18n_1.tBi)('Credits', '积分')}</div>
                </div>` : ''}
                ${gmCalls > 0 ? `
                <div class="cal-overview-item">
                    <div class="cal-overview-val">${gmCalls}</div>
                    <div class="cal-overview-label">${(0, i18n_1.tBi)('GM Calls', 'GM 调用')}</div>
                </div>` : ''}
                ${errors > 0 ? `
                <div class="cal-overview-item">
                    <div class="cal-overview-val cal-day-total-danger">${errors}</div>
                    <div class="cal-overview-label">${(0, i18n_1.tBi)('Errors', '错误')}</div>
                </div>` : ''}
            </div>`;
}
//# sourceMappingURL=webview-calendar-tab.js.map