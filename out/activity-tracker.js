...
function stepDurationReasoning(meta) {
    const created = meta?.createdAt;
    if (!created) {
        return 0;
    }
    const end = (meta.finishedGeneratingAt || meta.viewableAt || meta.completedAt);
    if (!end) {
        return 0;
    }
    try {
        return Math.max(0, new Date(end).getTime() - new Date(created).getTime());
    }
    catch {
        return 0;
    }
}
function stepDurationTool(meta) {
    const created = meta?.createdAt;
    if (!created) {
        return 0;
    }
    const end = (meta.completedAt || meta.finishedGeneratingAt || meta.viewableAt);
    if (!end) {
        return 0;
    }
    try {
    ...
    try {
        ...
        if (argsStr) {
            ...
            switch (type) {
                ...
                case 'MCP_TOOL': {
                    const tn = toolCall?.name || '';
                    return tn.replace(/^mcp_/, '').replace(/^github-mcp-server_/, 'gh/');
                }
        ...
        const viewFile = step.viewFile;
        if (viewFile?.absolutePathUri) {
            return viewFile.absolutePathUri.replace('file:///', '').split(/[\\/]/).pop() || '';
        }
...
function extractToolName(step, fallbackLabel) {
    try {
        const meta = step.metadata;
        const toolCall = meta?.toolCall;
        ...
        if (name) {
            ...
            if (name.startsWith('mcp_github-mcp-server_')) {
                return 'gh/' + name.replace('mcp_github-mcp-server_', '');
            }
            if (name.startsWith('mcp_web-fetcher_')) {
                return name.replace('mcp_web-fetcher_', '');
            }
            if (name.startsWith('mcp_memory-store_')) {
                return name.replace('mcp_memory-store_', '');
            }
            if (name.startsWith('mcp_sandbox_')) {
                return name.replace('mcp_sandbox_', '');
            }
            if (name.startsWith('mcp_sequential-thinking_')) {
                return 'thinking';
            }
...
function sameTriggeredByScope(a, b) {
...
function buildGMEventKey(cascadeId, stepIndex) {
    return `${cascadeId || 'unknown'}::${stepIndex ?? -1}`;
}
function buildRawStepFingerprint(step, cascadeId) {
    const type = step.type || '';
    const meta = (step.metadata || {});
    const createdAt = meta.createdAt || '';
    if (!cascadeId || !type || !createdAt) {
        return undefined;
    }
    return `${cascadeId}|${type}|${createdAt}`;
}
function buildLegacyStepEventIdentity(event) {
    return [
        event.cascadeId || '',
        event.source || '',
        event.category,
...
function isLowSignalPromptSnippet(snippet) {
    ...
    if (/^bot-[0-9a-f-]{8,}$/i.test(clean)) {
        return true;
    }
    if (/^toolu_[a-z0-9_-]{8,}$/i.test(clean)) {
        return true;
    }
    if (/^req_vrtx_[a-z0-9_-]{8,}$/i.test(clean)) {
        return true;
    }
    if (/^Step Id:\s*\d+\s+\{\{\s*CHECKPOINT/i.test(clean)) {
        return true;
    }
    if (/earlier parts of this conversation/i.test(clean)) {
        return true;
    }
    if (/conversation have been compressed/i.test(clean)) {
        return true;
    }
    return false;
}
 ...
 */
function extractNotifyMessage(toolCalls) {
...
function mergeActivityStats(target, source) {
...
function mergeGMStats(target, source) {
...
function normalizeStepsByModelRecord(stepsByModel) {
    const normalized = {};
    for (const [modelName, steps] of Object.entries(stepsByModel)) {
        const key = (0, models_1.normalizeModelDisplayName)(modelName) || modelName;
...
class ActivityTracker {
    // Model stats
    _modelStats = new Map();
    _subAgentTokens = new Map();
    _checkpointHistory = [];
    _conversationBreakdown = new Map();
    _globalToolStats = new Map();
     ...
     */
    async processTrajectories(ls, trajectories, signal) {
        ...
        for (const t of trajectories) {
            trajMap.set(t.cascadeId, {
                ...
                status: t.status,
                requestedModel: t.requestedModel ? (0, models_1.normalizeModelDisplayName)(t.requestedModel) : '',
                generatorModel: t.generatorModel ? (0, models_1.normalizeModelDisplayName)(t.generatorModel) : '',
            });
        }
        ...
        if (!this._warmedUp) {
            ...
            for (const [id, info] of trajMap) {
                ...
                try {
                    const sr = await (0, rpc_client_1.rpcCall)(ls, 'GetCascadeTrajectorySteps', { cascadeId: id, startIndex: 0, endIndex: sc }, 15000, signal);
                    const allSteps = (sr.steps || []);
        ...
        for (const [id, info] of trajMap) {
            ...
            if (currSteps < entry.stepCount) {
                ...
                this._recentSteps = this._recentSteps.filter(ev => ev.cascadeId !== id || ev.stepIndex === undefined || ev.stepIndex < currSteps);
            }
            ...
            try {
                ...
                if (entry.processedIndex === 0) {
                    ...
                    const sr = await (0, rpc_client_1.rpcCall)(ls, 'GetCascadeTrajectorySteps', { cascadeId: id, startIndex: 0, endIndex: currSteps }, 15000, signal);
    ...
    // ─── Step Processing ─────────────────────────────────────────────────
    _processStep(step, emitEvent = true, stepIndex, contextModel, cascadeId) {
        const type = step.type || '';
        const meta = (step.metadata || {});
        const modelId = meta.generatorModel || '';
        const model = (0, models_1.normalizeModelDisplayName)(modelId);
        const cls = classifyStep(type);
        const dur = cls.category === 'tool' ? stepDurationTool(meta) : stepDurationReasoning(meta);
        const observedAt = meta.createdAt || new Date().toISOString();
        ...
        if (type === 'CORTEX_STEP_TYPE_CHECKPOINT') {
            ...
            if (mu) {
                ...
                const attrModel = (0, models_1.normalizeModelDisplayName)(contextModel || model || mu.model || '');
                if (attrModel) {
                ...
                const rawDisplay = (0, models_1.normalizeModelDisplayName)(rawModel);
        ...
        if (type === 'CORTEX_STEP_TYPE_NOTIFY_USER') {
            ...
            const text = (nu.notificationContent || nu.message || '').trim();
            if (text && emitEvent) {
                this._upsertStepTimelineEvent({
                    timestamp,
                    icon: '📢',
        ...
        if (cls.category === 'reasoning') {
            ...
            const tdStr = pr?.thinkingDuration;
            ...
            let resp = (pr?.modifiedResponse || pr?.response || '');
            // Fallback: extract notify_user Message from toolCalls (actual AI reply)
            const notifyMsg = extractNotifyMessage(toolCalls);
            if (!resp && notifyMsg) {
                resp = notifyMsg;
            ...
            const tokens = parseInt(meta.toolCallOutputTokens || '0', 10);
            if (tokens > 0) {
                s.toolReturnTokens += tokens;
            }
     ...
     */
    _injectTimelineEvent(step, stepIndex, cascadeId) {
    ...
    _buildStepTimelineEvent(step, stepIndex, cascadeId) {
        const type = step.type || '';
        const meta = (step.metadata || {});
        const modelId = meta.generatorModel || '';
        const model = (0, models_1.normalizeModelDisplayName)(modelId);
        ...
        if (cls.category === 'reasoning') {
            ...
            const tdStr = pr?.thinkingDuration;
            let aiResp = (pr?.modifiedResponse || pr?.response || '');
            const notifyMsg = extractNotifyMessage(toolCalls);
            if (!aiResp && notifyMsg) {
                aiResp = notifyMsg;
                detail = '';
    ...
    _mergeStepTimelineEvent(existing, event) {
        let changed = false;
        const mergeKeys = [
            'timestamp',
            'icon',
    ...
    _sanitizeUserTimelineEvent(event) {
    ...
    _upsertStepTimelineEvent(event) {
    ...
    _refreshTimelineTail(steps, absOffset, cascadeId) {
        if (steps.length === 0) {
            return false;
        }
    ...
    _removeStepTimelineEvent(cascadeId, stepIndex, rawStep) {
        const fingerprint = rawStep ? buildRawStepFingerprint(rawStep, cascadeId) : undefined;
        const category = rawStep ? classifyStep(rawStep.type || '').category : undefined;
        const before = this._recentSteps.length;
        this._recentSteps = this._recentSteps.filter(ev => !(ev.source === 'step'
    ...
    _hasPendingPlannerSteps(cascadeId) {
        return (this._pendingPlannerSteps.get(cascadeId)?.size || 0) > 0;
    }
    _markPendingPlannerStep(cascadeId, stepIndex) {
    ...
    _clearPendingPlannerStep(cascadeId, stepIndex) {
        if (!cascadeId || stepIndex === undefined) {
            return;
        }
        const pending = this._pendingPlannerSteps.get(cascadeId);
    ...
    _clearPendingPlannerStepsFrom(cascadeId, minStepIndexExclusive) {
        const pending = this._pendingPlannerSteps.get(cascadeId);
        if (!pending) {
            return;
        }
    ...
    _trackSample(model, cat) {
        this._sampleTotal++;
        const normalizedModel = (0, models_1.normalizeModelDisplayName)(model) || model;
        let d = this._sampleDist.get(normalizedModel);
        if (!d) {
            d = { reasoning: 0, toolCalls: 0, errors: 0, other: 0 };
            this._sampleDist.set(normalizedModel, d);
        }
        d[cat]++;
    }
    /**
    ...
    _updateConversationBreakdown(cascadeId, steps) {
    ...
    injectGMData(gmSummary) {
        ...
        if (gmSummary) {
            ...
            for (const [name, stats] of Object.entries(gmSummary.modelBreakdown)) {
                const normalizedName = (0, models_1.normalizeModelDisplayName)(name) || name;
        ...
        const gmByStep = new Map();
        const virtualEvents = [];
        const userAnchorEvents = [];
        const seenVirtualKeys = new Set();
        const seenUserAnchors = new Set();
        for (const conv of gmSummary.conversations) {
            ...
            const outsideCalls = conv.calls.filter(call => call.stepIndices.length > 0
            ...
            const recoveredStepsByModel = {};
            for (const call of sortedOutsideCalls) {
                const modelName = (0, models_1.normalizeModelDisplayName)(call.modelDisplay || call.model) || call.responseModel || call.modelDisplay || call.model;
                if (!modelName) {
                    continue;
                }
            ...
            for (const call of sortedOutsideCalls) {
                for (const anchor of call.userMessageAnchors) {
                    if (anchor.stepIndex < visibleStepCount) {
                        continue;
                    }
                    // Filter out EPHEMERAL/system messages that GM captures as user anchors
                    const anchorText = (anchor.text || '').trim();
                    if (anchorText.startsWith('The following is an <EPHEMERAL_MESSAGE>')) {
                        continue;
                    }
                    if (anchorText.startsWith('Step Id:') && /CHECKPOINT/.test(anchorText)) {
                        continue;
                    }
                    const anchorKey = `${conv.cascadeId}:${anchor.stepIndex}`;
                    if (seenUserAnchors.has(anchorKey)) {
                        continue;
                    }
                    ...
                    if (seenUserAnchors.has(anchorTextKey)) {
                        continue;
                    }
                    seenUserAnchors.add(anchorTextKey);
                    const nextCall = sortedOutsideCalls.find(candidate => candidate.stepIndices.length > 0
            ...
            for (const call of outsideCalls) {
                ...
                virtualEvents.push({
                    ...
                    icon: '🧠',
                    category: 'reasoning',
                    model: (0, models_1.normalizeModelDisplayName)(call.modelDisplay || call.model) || call.modelDisplay || call.model || '',
        ...
        for (const ev of this._recentSteps) {
            ...
            if (ev.stepIndex === undefined || !ev.cascadeId) {
                continue;
            }
            ...
            if (gm.modelDisplay && (ev.source === 'estimated' || ev.source === 'gm_virtual' || gm.modelAccuracy === 'exact')) {
                ev.model = (0, models_1.normalizeModelDisplayName)(gm.modelDisplay || gm.model) || gm.modelDisplay || gm.model;
            }
        ...
        for (const ev of this._recentSteps) {
            ...
            timelineChanged = true;
            const modelNames = [...new Set(matchedCalls
                    .map(call => (0, models_1.normalizeModelDisplayName)(call.modelDisplay || call.model) || call.modelDisplay || call.model)
                    .filter(Boolean))];
        ...
        this._recentSteps = this._recentSteps.filter(ev => ev.source !== 'estimated' || !ev.estimatedResolved);
        // Filter gm_user anchors whose text duplicates an existing step-source user event.
        const stepUserTexts = new Set(this._recentSteps
            .filter(ev => ev.source === 'step' && ev.category === 'user' && ev.userInput)
            .map(ev => (ev.userInput || '').trim().slice(0, 120)));
        const dedupedUserAnchors = userAnchorEvents.filter(ev => !stepUserTexts.has((ev.userInput || '').trim().slice(0, 120)));
        ...
        for (const conv of gmSummary.conversations) {
            ...
            if (!dominantModel && conv.calls.length > 0) {
                ...
                for (const c of conv.calls) {
                    const dm = (0, models_1.normalizeModelDisplayName)(c.modelDisplay || c.model) || c.responseModel || c.modelDisplay || c.model || '';
                    if (dm) {
                        freq.set(dm, (freq.get(dm) || 0) + 1);
                    }
                }
                let topCount = 0;
            ...
            for (const call of conv.calls) {
                ...
                const callDisplay = (0, models_1.normalizeModelDisplayName)(call.modelDisplay || call.model) || call.responseModel || call.modelDisplay || call.model || '';
                if (!callDisplay || callDisplay === dominantModel) {
                    continue;
                }
        ...
        const visibleBoundary = Math.max(0, ...Array.from(this._trajectories.values()).map(entry => entry.processedIndex));
        if (!alreadyInjected && gmSummary.contextGrowth.length > 0 && visibleBoundary > 0) {
            ...
            if (outsideGrowth.length > 0) {
                ...
                const virtualHistory = outsideGrowth.map(pt => ({
    ...
    getSummary() {
        ...
        const subAgentTokens = [...subAgentMerged.values()];
        ...
        let gmAnnotated = 0;
        const gmSeen = new Set(); // deduplicate by cascadeId+stepIndex
        for (const ev of this._recentSteps) {
        ...
        const gmEligible = this._recentSteps.filter(e => e.category === 'reasoning' || e.category === 'tool').length;
        return {
            totalUserInputs,
            totalReasoning,
            totalToolCalls,
    ...
    getStatusBarText() {
        ...
        const parts = [];
        if (s.totalReasoning > 0) {
            parts.push(`🧠${s.totalReasoning}`);
        }
        if (s.totalToolCalls > 0) {
            parts.push(`⚡${s.totalToolCalls}`);
        }
        if (s.estSteps > 0) {
            parts.push(`📊+${s.estSteps}`);
        }
     ...
     */
    getModelStatusBarText(modelDisplayName) {
        this._normalizeModelState();
        const ms = this._modelStats.get((0, models_1.normalizeModelDisplayName)(modelDisplayName) || modelDisplayName);
        if (!ms || ms.totalSteps === 0) {
            return this.getStatusBarText();
        }
        const parts = [];
        if (ms.reasoning > 0) {
            parts.push(`🧠${ms.reasoning}`);
        }
        if (ms.toolCalls > 0) {
            parts.push(`⚡${ms.toolCalls}`);
        }
        if (ms.estSteps > 0) {
            parts.push(`📊+${ms.estSteps}`);
        }
    ...
    get isReady() { return this._warmedUp; }
    /** Get archived snapshots */
    getArchives() { return [...this._archives]; }
    /**
     ...
     */
    archiveAndReset(modelIds, options) {
        this._normalizeModelState();
        ...
        const poolDisplayNames = new Set();
        if (modelIds && modelIds.length > 0) {
            for (const id of modelIds) {
                poolDisplayNames.add((0, models_1.normalizeModelDisplayName)(id));
            }
        }
        ...
        let archiveSummary = fullSummary;
        if (isPoolReset) {
            // Filter modelStats to pool models only
            const poolModelStats = {};
            ...
            const poolSteps = fullSummary.recentSteps.filter(ev => !ev.model || poolDisplayNames.has(ev.model));
            archiveSummary = {
                ...fullSummary,
                ...
                subAgentTokens: archiveSummary.subAgentTokens.filter(entry => !!entry.ownerModel && poolDisplayNames.has(entry.ownerModel)),
                // Filter GM breakdown to pool models
                gmModelBreakdown: fullSummary.gmModelBreakdown
                    ? Object.fromEntries(Object.entries(fullSummary.gmModelBreakdown)
        ...
        if (isPoolReset) {
            ...
            this._recentSteps = this._recentSteps.filter(ev => ev.model && !poolDisplayNames.has(ev.model));
            ...
            for (const [cascadeId, attribution] of [...this._windowOutsideAttribution.entries()]) {
                const remaining = Object.fromEntries(Object.entries(attribution.stepsByModel)
    ...
    serialize() {
        const baselines = {};
        for (const [k, v] of this._trajectories) {
            baselines[k] = { ...v };
        }
        const windowOutsideAttribution = {};
    ...
    static restore(data) {
        ...
        if (Array.isArray(s.checkpointHistory)) {
        ...
        if (Array.isArray(s.conversationBreakdown)) {
        ...
        if (data.trajectoryBaselines) {
            for (const [id, baseline] of Object.entries(data.trajectoryBaselines)) {
                tracker._trajectories.set(id, {
                    ...
                    dominantModel: baseline.dominantModel || '',
                    lastStatus: baseline.lastStatus || '',
                    requestedModel: baseline.requestedModel || '',
                    generatorModel: baseline.generatorModel || '',
                });
            }
        }
    ...
    _normalizeModelState() {
        ...
        if (this._recentSteps.length > 0) {
            this._recentSteps = this._recentSteps.map(event => event.model
                ? { ...event, model: (0, models_1.normalizeModelDisplayName)(event.model) || event.model }
                : event);
        }
        if (this._gmModelBreakdown) {
            const merged = {};
        ...
        const normalizeSubAgentMap = (source) => {
            const normalized = new Map();
            for (const entry of source.values()) {
                const displayName = (0, models_1.normalizeModelDisplayName)(entry.displayName || entry.modelId) || entry.displayName || entry.modelId;
                const ownerModel = entry.ownerModel ? ((0, models_1.normalizeModelDisplayName)(entry.ownerModel) || entry.ownerModel) : undefined;
        ...
        if (this._trajectories.size > 0) {
            for (const [cascadeId, baseline] of this._trajectories) {
                this._trajectories.set(cascadeId, {
                    ...baseline,
                    dominantModel: (0, models_1.normalizeModelDisplayName)(baseline.dominantModel) || baseline.dominantModel,
                    requestedModel: (0, models_1.normalizeModelDisplayName)(baseline.requestedModel) || baseline.requestedModel,
                    generatorModel: (0, models_1.normalizeModelDisplayName)(baseline.generatorModel) || baseline.generatorModel,
                });
            }
        }
    }
    _getOrCreateStats(model) {
        const normalizedModel = (0, models_1.normalizeModelDisplayName)(model) || model;
    ...
    _applyWindowOutsideDelta(stepsByModel, direction) {
    ...
    _reconcileWindowOutsideAttribution(cascadeId, basis, stepsByModel) {
    ...
    _clearWindowOutsideAttribution(cascadeId) {
        const existing = this._windowOutsideAttribution.get(cascadeId);
        if (!existing) {
            return;
        }
