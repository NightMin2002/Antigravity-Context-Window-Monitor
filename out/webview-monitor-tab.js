...
exports.buildMonitorSections = buildMonitorSections;
...
function getConversationData(gmSummary, gmConversations, cascadeId) {
    if (gmSummary) {
        return gmSummary.conversations.find(c => c.cascadeId === cascadeId) || null;
    }
    return gmConversations?.[cascadeId] || null;
}
function aggregateGMForSession(gmSummary, gmConversations, cascadeId) {
    ...
    const lifetimeCalls = Math.max(liveConv?.lifetimeCalls ?? liveConv?.calls.length ?? 0, storedConv?.lifetimeCalls ?? storedConv?.calls.length ?? 0, liveConv?.calls.length ?? 0, storedConv?.calls.length ?? 0);
    const conv = liveConv || storedConv;
    const currentCalls = liveConv ? liveConv.calls : (storedConv?.calls || []);
    if ((!conv || currentCalls.length === 0) && lifetimeCalls === 0) {
        return empty;
    }
    ...
    for (const c of currentCalls) {
        ...
        if (c.ttftSeconds > 0) {
            ttftSum += c.ttftSeconds;
            ttftN++;
        }
        if (c.streamingSeconds > 0) {
            streamSum += c.streamingSeconds;
            streamN++;
        }
...
function buildMonitorSections(usage, allUsages, configs, userInfo, gmSummary, gmConversations, tracker, pricingStore) {
    const sections = [];
...
function buildOverviewGrid(sections, usage, configs, gmSummary, tracker, pricingStore) {
    const cards = [
...
function buildQuotaOverviewCard(usage, configs) {
    ...
    const rows = sorted.map(c => {
        ...
        const countdown = qi.resetTime ? (0, reset_time_1.formatResetCountdown)(qi.resetTime) : '';
        return `<div class="monitor-quota-item">
            <div class="monitor-quota-top">
                <span class="monitor-quota-name">${(0, webview_helpers_1.esc)(c.label)}</span>
            ...
            <div class="monitor-quota-meta">${countdown || (0, i18n_1.tBi)('No reset time', '无重置时间')}</div>
        </div>`;
    }).join('');
    const chips = [];
    if (currentConfig?.quotaInfo) {
        chips.push(`<span class="monitor-mini-tag">${(0, i18n_1.tBi)('Current', '当前')} ${(0, webview_helpers_1.esc)(currentConfig.label)} ${formatPercent(currentConfig.quotaInfo.remainingFraction)}</span>`);
    }
    if (lowest?.quotaInfo && (!currentConfig || lowest.model !== currentConfig.model)) {
        chips.push(`<span class="monitor-mini-tag is-warn">${(0, i18n_1.tBi)('Lowest', '最低')} ${(0, webview_helpers_1.esc)(lowest.label)} ${formatPercent(lowest.quotaInfo.remainingFraction)}</span>`);
    }
    if (nextReset?.quotaInfo?.resetTime) {
        chips.push(`<span class="monitor-mini-tag">${(0, i18n_1.tBi)('Next reset', '最近重置')} ${(0, reset_time_1.formatResetCountdown)(nextReset.quotaInfo.resetTime)}</span>`);
    }
    return `
        <section class="card monitor-summary-card monitor-summary-card-quota">
            <div class="monitor-summary-head">
                <div>
                    <div class="monitor-summary-kicker">${(0, i18n_1.tBi)('Quota Overview', '额度概览')}</div>
                    <h2>${webview_icons_1.ICON.bolt} ${(0, i18n_1.tBi)('Model Quota', '模型配额')}</h2>
                </div>
                <div class="monitor-summary-note">${quotaModels.length} ${(0, i18n_1.tBi)('models', '个模型')}</div>
            </div>
            ${chips.length > 0 ? `<div class="monitor-mini-tags">${chips.join('')}</div>` : ''}
            <div class="monitor-quota-list">${rows}</div>
        </section>`;
}
function buildGMOverviewCard(gmSummary) {
    if (!gmSummary || gmSummary.totalCalls <= 0) {
        return `
            <section class="card monitor-summary-card">
                <div class="monitor-summary-head">
                    <div>
                        <div class="monitor-summary-kicker">${(0, i18n_1.tBi)('GM Snapshot', 'GM 快照')}</div>
                        <h2>${webview_icons_1.ICON.bolt} ${(0, i18n_1.tBi)('GM Overview', 'GM 总览')}</h2>
                    </div>
                </div>
                <p class="monitor-summary-empty">${(0, i18n_1.tBi)('GM data will appear automatically after the first precise call is captured.', '捕捉到第一批精确 GM 调用后，这里会自动显示。')}</p>
            </section>`;
    }
    ...
    const topModels = Object.entries(gmSummary.modelBreakdown)
        ...
        return `<div class="monitor-gm-model-item">
                <div class="monitor-gm-model-name">${(0, webview_helpers_1.esc)(name)}</div>
    ...
    return `
        <section class="card monitor-summary-card monitor-summary-card-gm">
            ...
            <div class="stat-grid">
                <div class="stat">
                    <div class="stat-label">${(0, i18n_1.tBi)('Calls', '调用')}</div>
                    <div class="stat-value">${gmSummary.totalCalls}</div>
                </div>
                <div class="stat">
                    <div class="stat-label">${(0, i18n_1.tBi)('Input', '输入')}</div>
                    <div class="stat-value">${(0, statusbar_1.formatTokenCount)(gmSummary.totalInputTokens)}</div>
                </div>
                <div class="stat">
                    <div class="stat-label">${(0, i18n_1.tBi)('Output', '输出')}</div>
                    <div class="stat-value">${(0, statusbar_1.formatTokenCount)(gmSummary.totalOutputTokens)}</div>
                </div>
                <div class="stat">
                    <div class="stat-label">${(0, i18n_1.tBi)('Thinking', '思考')}</div>
                    <div class="stat-value">${(0, statusbar_1.formatTokenCount)(gmSummary.totalThinkingTokens)}</div>
                </div>
                <div class="stat">
                    <div class="stat-label">${(0, i18n_1.tBi)('Avg TTFT', '平均 TTFT')}</div>
                    <div class="stat-value">${avgTTFT > 0 ? `${avgTTFT.toFixed(2)}s` : '—'}</div>
                </div>
                <div class="stat">
                    <div class="stat-label">${(0, i18n_1.tBi)('Cache Hit', '缓存命中')}</div>
                    <div class="stat-value">${Math.round(cacheHitRate * 100)}%</div>
                </div>
            </div>
            <div class="monitor-mini-tags">
                <span class="monitor-mini-tag">${(0, i18n_1.tBi)('Exact', '精确调用')} ${Math.round(exactRate * 100)}%</span>
                <span class="monitor-mini-tag">${(0, i18n_1.tBi)('Streaming', '流式')} ${avgStreaming > 0 ? `${avgStreaming.toFixed(2)}s` : '—'}</span>
                <span class="monitor-mini-tag">${(0, i18n_1.tBi)('Models', '模型数')} ${modelStats.length}</span>
            </div>
            ${topModels ? `
                <div class="monitor-inline-section">
                    <div class="monitor-inline-title">${(0, i18n_1.tBi)('Top Models by Calls', '模型调用分布')}</div>
                    <div class="monitor-gm-model-grid">${topModels}</div>
                </div>` : ''}
        </section>`;
}
function buildCostOverviewCard(gmSummary, pricingStore) {
    if (!pricingStore || !gmSummary || gmSummary.totalCalls <= 0) {
        return `
            <section class="card monitor-summary-card">
                <div class="monitor-summary-head">
                    <div>
                        <div class="monitor-summary-kicker">${(0, i18n_1.tBi)('Cost Snapshot', '成本快照')}</div>
                        <h2>${IC.coin} ${(0, i18n_1.tBi)('Cost Overview', '成本速览')}</h2>
                    </div>
                </div>
                <p class="monitor-summary-empty">${(0, i18n_1.tBi)('Cost overview will appear after GM data and pricing are both available.', 'GM 数据和价格表都就绪后，这里会自动显示成本速览。')}</p>
            </section>`;
    }
    ...
    const modelList = rows.map(row => {
        ...
        return `<div class="monitor-cost-model">
            ...
            <div class="monitor-cost-model-main">
                <span class="monitor-cost-model-name">${(0, webview_helpers_1.esc)(row.name)}</span>
                <span class="monitor-cost-model-calls">${stats?.callCount ?? 0} ${(0, i18n_1.tBi)('calls', '次调用')}${ratioLabel ? ` · ${ratioLabel}` : ''}</span>
            </div>
            <div class="monitor-cost-model-side">
                <span class="monitor-cost-model-cost">${row.pricing ? formatUsd(row.totalCost) : (0, i18n_1.tBi)('Unpriced', '未定价')}</span>
            </div>
        </div>`;
    }).join('');
    return `
        <section class="card monitor-summary-card monitor-summary-card-cost">
            ...
            <div class="stat-grid three-col">
                <div class="stat">
                    <div class="stat-label">${(0, i18n_1.tBi)('Cycle Total', '周期总计')}</div>
                    <div class="stat-value">${formatUsd(grandTotal)}</div>
                </div>
                <div class="stat">
                    <div class="stat-label">${(0, i18n_1.tBi)('Priced Models', '已定价模型')}</div>
                    <div class="stat-value">${pricedRows.length}</div>
                </div>
                <div class="stat">
                    <div class="stat-label">${(0, i18n_1.tBi)('Most Expensive', '最高成本')}</div>
                    <div class="stat-value">${topCost ? formatUsd(topCost.totalCost) : '$0'}</div>
                </div>
            </div>
            <div class="monitor-mini-tags">
                ${topCost ? `<span class="monitor-mini-tag">${(0, i18n_1.tBi)('Top model', '最高模型')} ${(0, webview_helpers_1.esc)(topCost.name)}</span>` : ''}
                <span class="monitor-mini-tag">${(0, i18n_1.tBi)('Used models', '使用模型')} ${rows.length}</span>
            </div>
            <div class="monitor-cost-list">${modelList}</div>
        </section>`;
}
function buildQuotaTrackingOverviewCard(tracker) {
    ...
    const activeRows = active
        ...
        const color = pct <= 20 ? 'var(--color-danger)' : pct < 80 ? 'var(--color-warn)' : 'var(--color-ok)';
        const countdown = session.cycleResetTime ? (0, reset_time_1.formatResetCountdown)(session.cycleResetTime) : '';
        const absolute = session.cycleResetTime ? (0, reset_time_1.formatResetAbsolute)(session.cycleResetTime) : '';
        const elapsed = (0, webview_helpers_1.formatDuration)(Date.now() - Date.parse(session.startTime));
        const statusText = session.completed
            ? (0, i18n_1.tBi)('Completed, waiting for reset', '已耗尽，等待重置')
            : (0, i18n_1.tBi)('Tracking', '追踪中');
        return `<div class="monitor-track-row">
                <div class="monitor-track-main">
                    <div class="monitor-track-top">
                        <span class="monitor-track-name">${(0, webview_helpers_1.esc)(session.modelLabel)}</span>
                    ...
                    <div class="monitor-track-meta">
                        ...
                        <span class="monitor-mini-tag">${(0, i18n_1.tBi)('Elapsed', '已追踪')} ${elapsed}</span>
                        ${countdown ? `<span class="monitor-mini-tag">${(0, i18n_1.tBi)('Reset in', '重置剩余')} ${countdown}</span>` : ''}
                        ${absolute ? `<span class="monitor-mini-tag">${(0, i18n_1.tBi)('At', '时间')} ${absolute}</span>` : ''}
                    </div>
                </div>
            </div>`;
    }).join('');
    return `
        <section class="card monitor-summary-card monitor-summary-card-tracking monitor-tracking-card">
            <div class="monitor-summary-head">
                <div>
                    <div class="monitor-summary-kicker">${(0, i18n_1.tBi)('Tracking Snapshot', '追踪快照')}</div>
                    <h2>${webview_icons_1.ICON.timeline} ${(0, i18n_1.tBi)('Quota Tracking', '额度追踪')}</h2>
                </div>
                <div class="monitor-summary-note">${active.length} ${(0, i18n_1.tBi)('active', '活跃')}</div>
            </div>
            <div class="stat-grid three-col">
                <div class="stat">
                    <div class="stat-label">${(0, i18n_1.tBi)('Active', '活跃')}</div>
                    <div class="stat-value">${active.length}</div>
                </div>
                <div class="stat">
                    <div class="stat-label">${(0, i18n_1.tBi)('Archived', '归档')}</div>
                    <div class="stat-value">${history.length}</div>
                </div>
                <div class="stat">
                    <div class="stat-label">${(0, i18n_1.tBi)('Last Archived', '最近归档')}</div>
                    <div class="stat-value">${latestReset?.endTime ? (0, reset_time_1.formatResetAbsolute)(latestReset.endTime) : '—'}</div>
                </div>
            </div>
            ${activeRows
        ? `<div class="monitor-tracking-list">${activeRows}</div>`
        : `<p class="monitor-summary-empty">${(0, i18n_1.tBi)('Tracking is enabled, but there are no active quota sessions right now.', '额度追踪已启用，但当前没有活跃追踪会话。')}</p>`}
            ${latestReset?.endTime ? `<div class="monitor-mini-tags">
                <span class="monitor-mini-tag">${(0, i18n_1.tBi)('Latest archive', '最近归档')} ${(0, reset_time_1.formatResetAbsolute)(latestReset.endTime)}</span>
            </div>` : ''}
        </section>`;
}
function buildCurrentSessionSection(sections, usage, gm, gmConversations) {
    if (!usage) {
        sections.push(`
            <section class="card empty">
                <h2>${webview_icons_1.ICON.clock} ${(0, i18n_1.tBi)('Waiting for Session', '等待会话')}</h2>
                <p class="empty-desc">${(0, i18n_1.tBi)('Start a conversation in Antigravity to see usage data.', '在 Antigravity 中开始对话即可查看使用数据。')}</p>
            </section>`);
        return;
    }
    ...
    const sourceTag = usage.isEstimated
        ? `<span class="badge warn-badge">${(0, i18n_1.t)('panel.estimated')}</span>`
        : `<span class="badge ok-badge">${(0, i18n_1.t)('panel.preciseShort')}</span>`;
    // Compression alert
    const compressionStats = (0, statusbar_1.calculateCompressionStats)(usage);
    let compressHtml = '';
    if (compressionStats) {
        compressHtml = `
                <div class="compression-alert">
                    ${webview_icons_1.ICON.compress}
                    <span>${(0, i18n_1.t)('panel.compression')}: -${(0, statusbar_1.formatTokenCount)(compressionStats.dropTokens)} (${compressionStats.dropPercent.toFixed(1)}%)</span>
                </div>`;
    }
    // GM precision data
    ...
    if (gs.hasData && (gs.thinkingTokens > 0 || gs.responseTokens > 0)) {
        ...
        outputSplitHtml = `
                <details class="collapsible" id="d-output-split">
                    <summary>${IC.brain} ${(0, i18n_1.tBi)('Output Breakdown', '输出分拆')} <span class="badge ok-badge">GM</span></summary>
                    ...
                    <div class="split-legend">
                        <span><span class="dot thinking-dot"></span> ${(0, i18n_1.tBi)('Thinking', '思考')} ${(0, statusbar_1.formatTokenCount)(gs.thinkingTokens)} (${thinkPct}%)</span>
                        <span><span class="dot response-dot"></span> ${(0, i18n_1.tBi)('Response', '正文')} ${(0, statusbar_1.formatTokenCount)(gs.responseTokens)} (${respPct}%)</span>
                    </div>
                    </div>
                </details>`;
    }
    ...
    if (gs.hasData && gs.cacheRead > 0) {
        ...
        cacheHtml = `
                <details class="collapsible" id="d-cache-efficiency">
                    <summary>${IC.cache} ${(0, i18n_1.tBi)('Cache Efficiency', '缓存效率')} <span class="badge ok-badge">GM</span></summary>
                    <div class="details-body">
                    <div class="cache-row">
                        <div class="cache-ring-wrap">
                            ...
                            <div class="cache-ring-label">${(0, i18n_1.tBi)('Hit Rate', '命中率')}</div>
                        </div>
                        <div class="stat-grid three-col" style="flex:1">
                            <div class="stat mini">
                                <div class="stat-label">${(0, i18n_1.tBi)('Cache Read', '缓存读取')}</div>
                                <div class="stat-value">${(0, statusbar_1.formatTokenCount)(gs.cacheRead)}</div>
                            </div>
                            <div class="stat mini">
                                <div class="stat-label">${(0, i18n_1.tBi)('Cache Create', '缓存创建')}</div>
                                <div class="stat-value">${(0, statusbar_1.formatTokenCount)(gs.cacheCreate)}</div>
                            </div>
                            <div class="stat mini">
                                <div class="stat-label">${(0, i18n_1.tBi)('Hit Calls', '命中次数')}</div>
    ...
    const gmSessionStatsHtml = gs.hasData ? `
                <div class="stat-grid four-col">
                    ...
                    <div class="stat mini">
                        <div class="stat-label">${(0, i18n_1.tBi)('Lifetime', '累计调用')}</div>
                        <div class="stat-value">${gs.lifetimeCalls}</div>
                    </div>
                    <div class="stat mini">
                        <div class="stat-label">${(0, i18n_1.tBi)('Retry', '重试')}</div>
    ...
    if (usage.estimatedDeltaSinceCheckpoint > 0 && usage.lastModelUsage) {
        deltaHtml = `
                <div class="delta-hint">
                    ${(0, i18n_1.t)('tooltip.estDelta')}: +${usage.estimatedDeltaSinceCheckpoint.toLocaleString()} ${(0, i18n_1.tBi)('tokens', '令牌')} (${(0, i18n_1.t)('tooltip.sinceCheckpoint')})
                </div>`;
    }
    sections.push(`
            <section class="card">
                <h2>
                    ${webview_icons_1.ICON.clock}
                    ${(0, i18n_1.t)('panel.currentSession')}
                    ${sourceTag}
                    <span class="badge status-badge">${(0, webview_helpers_1.esc)(usage.status.replace('CASCADE_RUN_STATUS_', ''))}</span>
                </h2>
                <div class="stat-grid">
                    <div class="stat">
                        <div class="stat-label">${(0, i18n_1.t)('tooltip.model')}</div>
                        <div class="stat-value">${(0, webview_helpers_1.esc)(usage.modelDisplayName)}</div>
                    </div>
                    <div class="stat">
                        <div class="stat-label">${(0, i18n_1.t)('tooltip.session')}</div>
                        <div class="stat-value title-val">${(0, webview_helpers_1.esc)(usage.title || usage.cascadeId.substring(0, 8))}</div>
                    </div>
                </div>
                ${buildGitInfoHtml(usage)}
                <div class="progress-section">
                    <div class="progress-header">
                        <span>${(0, i18n_1.tBi)('Context Usage', '上下文使用')}</span>
                    ...
                    <div class="progress-detail">
                        ${(0, statusbar_1.formatTokenCount)(usage.contextUsed)} / ${(0, statusbar_1.formatContextLimit)(usage.contextLimit)}
                        <span class="dim">(${(0, i18n_1.t)('panel.remaining')}: ${(0, statusbar_1.formatTokenCount)(remaining)})</span>
                    </div>
                </div>
                <div class="stat-grid four-col">
                    <div class="stat mini">
                        <div class="stat-label">${(0, i18n_1.t)('tooltip.steps')}</div>
                        <div class="stat-value">${usage.stepCount}</div>
                    </div>
                    <div class="stat mini">
                        <div class="stat-label">${(0, i18n_1.t)('tooltip.modelOutput')}</div>
                        <div class="stat-value">${(0, statusbar_1.formatTokenCount)(usage.totalOutputTokens)}</div>
                    </div>
                    <div class="stat mini">
                        <div class="stat-label">${(0, i18n_1.t)('tooltip.toolResults')}</div>
                        <div class="stat-value">${(0, statusbar_1.formatTokenCount)(usage.totalToolCallOutputTokens)}</div>
                    </div>
                    <div class="stat mini">
                        <div class="stat-label">${(0, i18n_1.t)('tooltip.imageGen')}</div>
                ...
                <details class="collapsible" id="d-current-times">
                    <summary><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg> ${(0, i18n_1.tBi)('Timestamps', '时间戳')}</summary>
                    <div class="details-body">
                        <div class="ts-grid">
                            <div class="ts-card">
                                ...
                                <div class="ts-label">${(0, i18n_1.tBi)('Created', '创建')}</div>
                                <div class="ts-value">${(0, webview_helpers_1.formatTime)(usage.createdTime)}</div>
                            </div>
                            <div class="ts-card">
                                ...
                                <div class="ts-label">${(0, i18n_1.tBi)('Last Modified', '最后修改')}</div>
                                <div class="ts-value">${(0, webview_helpers_1.formatTime)(usage.lastModifiedTime)}</div>
                            </div>
                            <div class="ts-card">
                                ...
                                <div class="ts-label">${(0, i18n_1.tBi)('Last User Input', '最后用户输入')}</div>
                                <div class="ts-value">${(0, webview_helpers_1.formatTime)(usage.lastUserInputTime)}</div>
                            </div>
                            <div class="ts-card">
                                ...
                                <div class="ts-label">${(0, i18n_1.tBi)('Last Input Step', '最后输入步骤')}</div>
                                <div class="ts-value">#${usage.lastUserInputStepIndex}</div>
                            </div>
                        </div>
                        <div class="ts-cascade">
                            <span class="ts-cascade-label">${(0, i18n_1.tBi)('Cascade ID', '对话 ID')}</span>
                            <span class="mono-val">${(0, webview_helpers_1.esc)(usage.cascadeId)}</span>
                        </div>
                    </div>
                </details>
            </section>`);
}
function buildOtherSessionsSection(sections, currentUsage, allUsages, gm, gmConversations) {
    ...
    const rows = others.slice(0, 10).map((u, idx) => {
        ...
        const statusTag = `<span class="badge status-badge">${(0, webview_helpers_1.esc)(u.status.replace('CASCADE_RUN_STATUS_', ''))}</span>`;
        const remaining = Math.max(0, u.contextLimit - u.contextUsed);
        const sourceTag = u.isEstimated
            ? `<span class="badge warn-badge">${(0, i18n_1.tBi)('EST', '估')}</span>`
        ...
        if (gs.hasData) {
            const parts = [];
            if (gs.calls > 0) {
                parts.push(`${gs.calls} ${(0, i18n_1.tBi)('calls', '调用')}`);
            }
            if (gs.lifetimeCalls > 0) {
                parts.push(`${(0, i18n_1.tBi)('Lifetime', '累计')} ${gs.lifetimeCalls}`);
            }
            if (gs.cacheHitRate > 0) {
                parts.push(`${(0, i18n_1.tBi)('Cache', '缓存')} ${Math.round(gs.cacheHitRate * 100)}%`);
            }
            if (gs.retryCount > 0) {
                parts.push(`${(0, i18n_1.tBi)('Retry', '重试')} ${gs.retryCount}`);
            }
            if (gs.credits > 0) {
                parts.push(`${gs.credits} ${(0, i18n_1.tBi)('cr', '积分')}`);
            }
            if (gs.latestCallModel) {
                parts.push(`${(0, i18n_1.tBi)('Last', '最后')}: ${(0, webview_helpers_1.esc)(gs.latestCallModel)}`);
            }
            if (gs.aliasOnlyCalls > 0) {
                parts.push(`${(0, i18n_1.tBi)('Alias', '别名')} ${gs.aliasOnlyCalls}`);
            }
        ...
        if (gs.hasData && (gs.thinkingTokens > 0 || gs.responseTokens > 0)) {
            ...
            splitMini = `
                        <div class="stat mini">
                            <div class="stat-label">${(0, i18n_1.tBi)('Thinking', '思考')}</div>
                            <div class="stat-value">${(0, statusbar_1.formatTokenCount)(gs.thinkingTokens)} (${thinkPct}%)</div>
                        </div>`;
        }
        return `
                <details class="collapsible session-detail" id="d-session-${idx}">
                    <summary>
                        <div class="session-summary-row">
                            <span class="session-title-text">${(0, webview_helpers_1.esc)(u.title || u.cascadeId.substring(0, 8))}</span>
                    ...
                    <div class="details-body">
                        <div class="stat-grid four-col">
                            <div class="stat mini">
                                <div class="stat-label">${(0, i18n_1.tBi)('Used', '已用')}</div>
                                <div class="stat-value">${(0, statusbar_1.formatTokenCount)(u.contextUsed)}</div>
                            </div>
                            <div class="stat mini">
                                <div class="stat-label">${(0, i18n_1.tBi)('Limit', '限制')}</div>
                                <div class="stat-value">${(0, statusbar_1.formatContextLimit)(u.contextLimit)}</div>
                            </div>
                            <div class="stat mini">
                                <div class="stat-label">${(0, i18n_1.tBi)('Remaining', '剩余')}</div>
                                <div class="stat-value">${(0, statusbar_1.formatTokenCount)(remaining)}</div>
                            </div>
                            <div class="stat mini">
                                <div class="stat-label">${(0, i18n_1.t)('tooltip.steps')}</div>
                        ...
                        <div class="stat-grid four-col">
                            <div class="stat mini">
                                <div class="stat-label">${(0, i18n_1.t)('tooltip.model')}</div>
                                <div class="stat-value" style="font-size:0.85em">${(0, webview_helpers_1.esc)(u.modelDisplayName)}</div>
                            </div>
                            <div class="stat mini">
                                <div class="stat-label">${(0, i18n_1.t)('tooltip.modelOutput')}</div>
                                <div class="stat-value">${(0, statusbar_1.formatTokenCount)(u.totalOutputTokens)}</div>
                            </div>
                            <div class="stat mini">
                                <div class="stat-label">${(0, i18n_1.t)('tooltip.toolResults')}</div>
                                <div class="stat-value">${(0, statusbar_1.formatTokenCount)(u.totalToolCallOutputTokens)}</div>
                            </div>
                        ...
                        <div class="ts-grid" style="margin-top:var(--space-2)">
                            <div class="ts-card">
                                <div class="ts-label">${(0, i18n_1.tBi)('Created', '创建')}</div>
                                <div class="ts-value">${(0, webview_helpers_1.formatTime)(u.createdTime)}</div>
                            </div>
                            <div class="ts-card">
                                <div class="ts-label">${(0, i18n_1.tBi)('Last Modified', '最后修改')}</div>
                                <div class="ts-value">${(0, webview_helpers_1.formatTime)(u.lastModifiedTime)}</div>
                            </div>
                            <div class="ts-card">
                                <div class="ts-label">${(0, i18n_1.tBi)('Last Input', '最后输入')}</div>
                                <div class="ts-value">${(0, webview_helpers_1.formatTime)(u.lastUserInputTime)}</div>
                            </div>
                            <div class="ts-card">
                                <div class="ts-label">${(0, i18n_1.tBi)('Cascade ID', '对话 ID')}</div>
                                <div class="ts-value mono-val" style="font-size:0.72em">${(0, webview_helpers_1.esc)(u.cascadeId.substring(0, 12))}…</div>
                            </div>
                        </div>
                    </div>
                </details>`;
    ...
    sections.push(`
            <section class="card">
                <h2>${webview_icons_1.ICON.chat} ${(0, i18n_1.t)('panel.otherSessions')} (${others.length})</h2>
                ${rows}
            </section>`);
}
// ─── Builder: Compression History ────────────────────────────────────────────
function buildCompressionHistorySection(gs, gm, gmConversations, cascadeId) {
    if (!gs.hasData) {
        return '';
    }
    const conv = getConversationData(gm, gmConversations, cascadeId);
    if (!conv || conv.calls.length < 2) {
        return '';
    }
    ...
    const rows = drops.map(d => {
        ...
        return `<div class="compress-card">
                    <div class="compress-card-header">
                        <span class="compress-steps">${webview_icons_1.ICON.compress} #${d.fromStep} → #${d.toStep}</span>
                        <span class="compress-drop">-${(0, statusbar_1.formatTokenCount)(d.drop)} (${pct}%)</span>
                    </div>
                    ...
                    <div class="compress-detail">
                        <span>${(0, statusbar_1.formatTokenCount)(d.fromTokens)}</span>
                        ...
                        <span>${(0, statusbar_1.formatTokenCount)(d.toTokens)}</span>
                    </div>
                </div>`;
    }).join('');
    return `
                <details class="collapsible" id="d-compression-history">
                    <summary>${webview_icons_1.ICON.compress} ${(0, i18n_1.tBi)('Compression History', '压缩历史')} (${drops.length}) <span class="badge ok-badge">GM</span></summary>
                    <div class="details-body">${rows}</div>
                </details>`;
}
// ─── Builder: Per-Call Details ────────────────────────────────────────────────
function buildCallDetailsSection(gs, gm, gmConversations, cascadeId) {
    if (!gs.hasData) {
        return '';
    }
    const conv = getConversationData(gm, gmConversations, cascadeId);
    if (!conv || conv.calls.length === 0) {
        return '';
    }
    ...
    const rows = reversed.map((c) => {
        ...
        const stopTag = abnormal ? `<span class="badge danger-badge">${(0, webview_helpers_1.esc)(sr)}</span>` : `<span class="dim">${(0, webview_helpers_1.esc)(sr)}</span>`;
        const retryTag = c.retries > 0 ? `<span class="badge warn-badge">${c.retries} ${(0, i18n_1.tBi)('retry', '重试')}</span>` : '';
        const accuracyTag = c.modelAccuracy === 'exact'
            ? `<span class="badge ok-badge">${(0, i18n_1.tBi)('Exact', '实际')}</span>`
            : `<span class="badge warn-badge">${(0, i18n_1.tBi)('Alias', '别名')}</span>`;
        // Build stat chips
        const chips = [];
        chips.push(`<span class="call-chip">${(0, i18n_1.tBi)('In', '输入')} ${(0, statusbar_1.formatTokenCount)(c.inputTokens)}</span>`);
        chips.push(`<span class="call-chip">${(0, i18n_1.tBi)('Out', '输出')} ${(0, statusbar_1.formatTokenCount)(c.outputTokens)}</span>`);
        if (c.thinkingTokens > 0) {
            chips.push(`<span class="call-chip thinking">${(0, i18n_1.tBi)('Think', '思考')} ${(0, statusbar_1.formatTokenCount)(c.thinkingTokens)}</span>`);
        }
        if (c.cacheReadTokens > 0) {
            chips.push(`<span class="call-chip cache">${(0, i18n_1.tBi)('Cache', '缓存')} ${(0, statusbar_1.formatTokenCount)(c.cacheReadTokens)}</span>`);
        }
        if (c.credits > 0) {
            chips.push(`<span class="call-chip">${c.credits} ${(0, i18n_1.tBi)('cr', '积分')}</span>`);
        }
        if (c.ttftSeconds > 0) {
            chips.push(`<span class="call-chip">TTFT ${c.ttftSeconds.toFixed(1)}s</span>`);
        }
        if (c.streamingSeconds > 0) {
            chips.push(`<span class="call-chip">${c.streamingSeconds.toFixed(1)}s</span>`);
        }
        return `<div class="call-card">
                    <div class="call-card-header">
                        <span class="call-idx">#${origIdx}</span>
                        <span class="call-model">${(0, webview_helpers_1.esc)(c.modelDisplay || c.responseModel || c.model)}</span>
    ...
    const header = skipped > 0
        ? `${(0, i18n_1.tBi)('LLM Call Details', 'LLM 调用明细')} (${(0, i18n_1.tBi)(`latest ${shown.length} of ${conv.calls.length}`, `最近 ${shown.length} / 当前 ${conv.calls.length}`)} · ${(0, i18n_1.tBi)('Lifetime', '累计')} ${gs.lifetimeCalls})`
        : `${(0, i18n_1.tBi)('LLM Call Details', 'LLM 调用明细')} (${(0, i18n_1.tBi)('Current', '当前')} ${conv.calls.length} · ${(0, i18n_1.tBi)('Lifetime', '累计')} ${gs.lifetimeCalls})`;
    return `
...
// ─── Helper: Git Info ────────────────────────────────────────────────────────
function buildGitInfoHtml(u) {
    if (!u.repositoryName && !u.branchName) {
        return '';
    }
    const parts = [];
    if (u.repositoryName) {
        parts.push(`<span class="git-repo">${webview_icons_1.ICON.git} ${(0, webview_helpers_1.esc)(u.repositoryName)}</span>`);
    }
    if (u.branchName) {
        parts.push(`<span class="git-branch">${webview_icons_1.ICON.branch} ${(0, webview_helpers_1.esc)(u.branchName)}</span>`);
    }
    return `<div class="git-info">${parts.join('')}</div>`;
}
//# sourceMappingURL=webview-monitor-tab.js.map