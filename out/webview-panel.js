...
exports.formatFileSize = exports.LARGE_STATE_FILE_WARN_BYTES = void 0;
exports.setPanelDurableState = setPanelDurableState;
exports.openUriInEditor = openUriInEditor;
exports.confirmLargeStateFileOpen = confirmLargeStateFileOpen;
exports.showMonitorPanel = showMonitorPanel;
exports.updateMonitorPanel = updateMonitorPanel;
exports.isMonitorPanelVisible = isMonitorPanelVisible;
const fs = __importStar(require("fs"));
const os = __importStar(require("os"));
const path = __importStar(require("path"));
const vscode = __importStar(require("vscode"));
const i18n_1 = require("./i18n");
const activity_panel_1 = require("./activity-panel");
...
const webview_monitor_tab_1 = require("./webview-monitor-tab");
const webview_models_tab_1 = require("./webview-models-tab");
const webview_profile_tab_1 = require("./webview-profile-tab");
const webview_settings_tab_1 = require("./webview-settings-tab");
const webview_history_tab_1 = require("./webview-history-tab");
const webview_calendar_tab_1 = require("./webview-calendar-tab");
const webview_chat_history_tab_1 = require("./webview-chat-history-tab");
const webview_script_1 = require("./webview-script");
...
let lastQuotaTracker;
let lastActivitySummary = null;
let lastActivityTracker;
let lastArchives = [];
let lastGMSummary = null;
let lastGMConversations = {};
let lastPricingStore;
let lastDailyStore;
let lastStorageDiagnostics;
let panelDurableState;
let lastModelDNA = {};
exports.LARGE_STATE_FILE_WARN_BYTES = 1 * 1024 * 1024;
/** Provide a durable state bucket for panel-level persistence (zoom, etc.). */
function setPanelDurableState(state) {
...
function refreshLocalStorageDiagnostics() {
    if (!lastStorageDiagnostics) {
        return;
    }
    ...
    lastStorageDiagnostics = {
        ...
        stateFileExists,
        stateFileSizeBytes,
        stateFileOpenWarnBytes: exports.LARGE_STATE_FILE_WARN_BYTES,
...
function clearDisposedPanel() {
    panel = undefined;
    isPaused = false;
}
async function openUriInEditor(target) {
    const options = {
        preview: false,
        preserveFocus: false,
...
function reportStateFileError(action, err) {
    const reason = err instanceof Error ? err.message : String(err);
    const warning = action === 'open'
        ? (0, i18n_1.tBi)('Failed to open state file.', '打开状态文件失败。')
        : (0, i18n_1.tBi)('Failed to reveal state file.', '定位状态文件失败。');
...
var webview_helpers_2 = require("./webview-helpers");
Object.defineProperty(exports, "formatFileSize", { enumerable: true, get: function () { return webview_helpers_2.formatFileSize; } });
async function confirmLargeStateFileOpen(fileSizeBytes) {
    if (fileSizeBytes < exports.LARGE_STATE_FILE_WARN_BYTES) {
        return 'open';
    }
    const openLabel = (0, i18n_1.tBi)('Open Anyway', '仍然打开');
    const revealLabel = (0, i18n_1.tBi)('Reveal Instead', '改为定位');
    const message = (0, i18n_1.tBi)(`The state file is ${(0, webview_helpers_1.formatFileSize)(fileSizeBytes)}. Opening it as plain text may stall the editor. Recommended: reveal it in the file manager instead.`, `状态文件大小为 ${(0, webview_helpers_1.formatFileSize)(fileSizeBytes)}。直接作为文本打开可能导致编辑器卡顿。更推荐先在文件管理器中定位它。`);
...
function getPanelHintPreferences() {
    return {
        showTabScrollHint: panelDurableState?.get('panelShowTabScrollHint', true) ?? true,
        showScrollbar: panelDurableState?.get('panelShowScrollbar', false) ?? false,
        showEndOfContent: panelDurableState?.get('panelShowEndOfContent', true) ?? true,
    };
}
function getAntigravityRoot() {
    return path.join(os.homedir(), '.gemini', 'antigravity');
}
function workspaceTargetFromUri(workspaceUri) {
    if (!workspaceUri) {
        return null;
    }
...
function getConversationTarget(cascadeId, kind) {
...
async function revealUriOrParent(target) {
    if (!target) {
        void vscode.window.showWarningMessage((0, i18n_1.tBi)('Unable to resolve that location.', '无法解析这个定位目标。'));
        return;
    }
    ...
    void vscode.window.showWarningMessage((0, i18n_1.tBi)('The target path does not exist yet.', '目标路径当前不存在。'));
}
function isDisposedWebviewError(err) {
 ...
 */
function showMonitorPanel(p) {
    lastUsage = p.currentUsage;
    lastAllUsages = p.allTrajectoryUsages;
    if (p.allTrajectories) {
        lastTrajectories = p.allTrajectories;
    }
    lastConfigs = p.modelConfigs;
    lastUserInfo = p.userInfo;
    if (p.workspaceUri) {
        lastWorkspaceUri = p.workspaceUri;
    }
    if (p.context) {
        extensionCtx = p.context;
    }
    if (p.tracker) {
        lastQuotaTracker = p.tracker;
    }
    if (p.activitySummary !== undefined) {
        lastActivitySummary = p.activitySummary;
    }
    if (p.archives) {
        lastArchives = p.archives;
    }
    if (p.activityTracker) {
        lastActivityTracker = p.activityTracker;
    }
    if (p.gmSummary !== undefined) {
        lastGMSummary = p.gmSummary;
    }
    if (p.gmConversations) {
        lastGMConversations = p.gmConversations;
    }
    if (p.pricingStore) {
        lastPricingStore = p.pricingStore;
    }
    if (p.dailyStore) {
        lastDailyStore = p.dailyStore;
    }
    if (p.storageDiagnostics) {
        lastStorageDiagnostics = p.storageDiagnostics;
    }
    if (p.modelDNA) {
        lastModelDNA = p.modelDNA;
    }
    if (panel) {
        ...
        if (p.initialTab) {
            setTimeout(() => safePostMessage({ command: 'switchToTab', tab: p.initialTab }), 100);
        }
        return;
    }
    panel = vscode.window.createWebviewPanel('antigravityMonitor', `${(0, i18n_1.tBi)('Antigravity Monitor', 'Antigravity 监控面板')}`, { viewColumn: vscode.ViewColumn.Two, preserveFocus: true }, { enableScripts: true });
    panel.webview.html = buildHtml(p.currentUsage, p.allTrajectoryUsages, p.modelConfigs, p.userInfo, isPaused, lastQuotaTracker);
    if (p.initialTab) {
        setTimeout(() => safePostMessage({ command: 'switchToTab', tab: p.initialTab }), 100);
    }
    panel.webview.onDidReceiveMessage(async (msg) => {
        if (msg.command === 'switchLanguage' && msg.lang && extensionCtx) {
            await (0, i18n_1.setLanguage)(msg.lang, extensionCtx);
            if (panel) {
            ...
            if (!stat) {
                const warning = (0, i18n_1.tBi)('State file has not been created yet.', '状态文件尚未生成。');
            ...
            if (decision === 'cancel') {
                safePostMessage({
                    ...
                    action: 'open',
                    ok: false,
                    message: (0, i18n_1.tBi)('Open cancelled.', '已取消打开。'),
                });
                return;
            }
            if (decision === 'reveal') {
            ...
            const zoom = clamp(Math.round(msg.value), 50, 200);
            if (panelDurableState) {
        ...
        else if (msg.command === 'switchCalendarMonth' && typeof msg.year === 'number') {
            calendarYear = msg.year;
            calendarMonth = msg.month;
            if (panel) {
                panel.webview.html = buildHtml(lastUsage, lastAllUsages, lastConfigs, lastUserInfo, isPaused, lastQuotaTracker);
            }
        }
    ...
    panel.onDidChangeViewState((e) => {
        if (e.webviewPanel.visible && !isPaused) {
            safePostMessage({
                command: 'updateTabs',
                tabs: buildTabContents(lastUsage, lastAllUsages, lastConfigs, lastUserInfo, lastQuotaTracker),
...
function buildTabContents(usage, allUsages, configs, userInfo, tracker) {
    const eoc = `<div class="eoc-sentinel"><span class="eoc-sentinel-text">${(0, i18n_1.tBi)('— End of content —', '— 已到底 —')}</span></div>`;
    return {
        monitor: (0, webview_monitor_tab_1.buildMonitorSections)(usage, allUsages, configs, userInfo, lastGMSummary, lastGMConversations, tracker, lastPricingStore) + eoc,
        gmdata: (0, activity_panel_1.buildGMDataTabContent)(lastActivitySummary, lastGMSummary, usage) + eoc,
        chats: (0, webview_chat_history_tab_1.buildChatHistoryTabContent)(lastTrajectories, usage, lastGMSummary, lastGMConversations, lastWorkspaceUri) + eoc,
        pricing: (lastPricingStore
            ? (0, pricing_panel_1.buildPricingTabContent)(lastGMSummary, lastPricingStore, lastDailyStore?.getMonthCostBreakdown(new Date().getFullYear(), new Date().getMonth() + 1))
            : `<p class="empty-msg">${(0, i18n_1.tBi)('Initializing...', '初始化中...')}</p>`) + eoc,
        models: (0, webview_models_tab_1.buildModelsTabContent)(userInfo, configs, lastGMSummary, lastModelDNA) + eoc,
        history: (0, webview_history_tab_1.buildHistoryHtml)(tracker) + eoc,
        calendar: (0, webview_calendar_tab_1.buildCalendarTabContent)(lastDailyStore ?? undefined, calendarYear, calendarMonth) + eoc,
        profile: (0, webview_profile_tab_1.buildProfileContent)(userInfo, configs) + eoc,
...
function isMonitorPanelVisible() {
    return panel !== undefined;
}
// ─── HTML Builder ─────────────────────────────────────────────────────────────
function buildHtml(usage, allUsages, configs, userInfo, paused = false, tracker) {
    const monitorHtml = (0, webview_monitor_tab_1.buildMonitorSections)(usage, allUsages, configs, userInfo, lastGMSummary, lastGMConversations, tracker, lastPricingStore);
    const gmDataHtml = (0, activity_panel_1.buildGMDataTabContent)(lastActivitySummary, lastGMSummary, usage);
    const chatsHtml = (0, webview_chat_history_tab_1.buildChatHistoryTabContent)(lastTrajectories, usage, lastGMSummary, lastGMConversations, lastWorkspaceUri);
    const pricingHtml = lastPricingStore
        ? (0, pricing_panel_1.buildPricingTabContent)(lastGMSummary, lastPricingStore, lastDailyStore?.getMonthCostBreakdown(new Date().getFullYear(), new Date().getMonth() + 1))
        : `<p class="empty-msg">${(0, i18n_1.tBi)('Initializing...', '初始化中...')}</p>`;
    const modelsHtml = (0, webview_models_tab_1.buildModelsTabContent)(userInfo, configs, lastGMSummary, lastModelDNA);
    const historyHtml = (0, webview_history_tab_1.buildHistoryHtml)(tracker);
    const calendarHtml = (0, webview_calendar_tab_1.buildCalendarTabContent)(lastDailyStore ?? undefined, calendarYear, calendarMonth);
    const profileHtml = (0, webview_profile_tab_1.buildProfileContent)(userInfo, configs);
    const settingsHtml = (0, webview_settings_tab_1.buildSettingsContent)(configs, tracker, lastStorageDiagnostics, getPanelHintPreferences());
...
<style>
${(0, webview_styles_1.getStyles)()}
${(0, activity_panel_1.getGMDataTabStyles)()}
${(0, pricing_panel_1.getPricingTabStyles)()}
${(0, webview_calendar_tab_1.getCalendarTabStyles)()}
</style>
</head>
<body data-privacy-default="true" data-zoom="${panelDurableState?.get('panelZoomLevel', 100) ?? 100}" data-tab-hint-enabled="${panelHintPrefs.showTabScrollHint ? 'true' : 'false'}" data-hide-scrollbar="${panelHintPrefs.showScrollbar ? 'false' : 'true'}" data-hide-eoc="${panelHintPrefs.showEndOfContent ? 'false' : 'true'}">
    <div class="panel-topbar">
        <header class="topbar-title">
            <h1>
                ${webview_icons_1.ICON.chart}
                ${(0, i18n_1.tBi)('Antigravity Monitor', 'Antigravity 监控面板')}
            </h1>
            <div class="header-actions">
                <div class="lang-switcher">
                    <button class="lang-btn${currentLang === 'zh' ? ' active' : ''}" data-lang="zh">中文</button>
                    <button class="lang-btn${currentLang === 'en' ? ' active' : ''}" data-lang="en">EN</button>
                    <button class="lang-btn${currentLang === 'both' ? ' active' : ''}" data-lang="both">${(0, i18n_1.tBi)('Both', '双语')}</button>
                </div>
                <button class="action-btn${paused ? ' paused' : ''}" id="pauseBtn" data-tooltip="${(0, i18n_1.tBi)(paused ? 'Resume auto-refresh' : 'Pause auto-refresh', paused ? '恢复自动刷新' : '暂停自动刷新')}">
        ...
        : '<path fill="currentColor" d="M5.5 3.5A1.5 1.5 0 0 1 7 5v6a1.5 1.5 0 0 1-3 0V5a1.5 1.5 0 0 1 1.5-1.5m5 0A1.5 1.5 0 0 1 12 5v6a1.5 1.5 0 0 1-3 0V5a1.5 1.5 0 0 1 1.5-1.5"/>'}</svg>
                </button>
                <button class="action-btn" id="refreshBtn" data-tooltip="${(0, i18n_1.tBi)('Refresh', '刷新')}">
                    ${webview_icons_1.ICON.refresh}
                </button>
                <span class="update-time">${paused ? `<span class="paused-indicator">${(0, i18n_1.tBi)('PAUSED', '已暂停')}</span>` : ''} ${new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit', second: '2-digit' })}</span>
            </div>
        </header>
        <div class="topbar-chips">
            <button class="info-chip chip-github" data-chip="github">
                ${webview_icons_1.ICON.git}
                <span>GitHub</span>
                ${webview_icons_1.ICON.externalLink}
            </button>
            <button class="info-chip chip-warn" data-chip="notice">
                ${webview_icons_1.ICON.windows}
                <span>${(0, i18n_1.tBi)('Notice', '提示')}</span>
            </button>
            <button class="info-chip chip-warn" data-chip="disclaimer">
                ...
                <span>${(0, i18n_1.tBi)('Disclaimer', '声明')}</span>
            </button>
        </div>
        <div class="chip-dropdown chip-dropdown-github" id="chip-github" hidden>
            <div class="chip-dropdown-content">
                <span class="chip-dropdown-text">
                    ${(0, i18n_1.tBi)('By <strong>AGI-is-going-to-arrive</strong> — open-source on GitHub. If you find it helpful, a', '作者 <strong>AGI-is-going-to-arrive</strong> — 项目已在 GitHub 开源。如果觉得有帮助，点个')}
                    <span class="star-inline">${webview_icons_1.ICON.star}</span>
                    ${(0, i18n_1.tBi)('would be appreciated.', '就是最大的支持。')}
                    <span class="heart-inline">${webview_icons_1.ICON.heart}</span>
                </span>
<a class="info-banner-link" href="https://github.com/AGI-is-going-to-arrive/Antigravity-Context-Window-Monitor" target="_blank" rel="noopener noreferrer">
                    ${webview_icons_1.ICON.externalLink} GitHub
                </a>
            </div>
        </div>
        <div class="chip-dropdown chip-dropdown-notice" id="chip-notice" hidden>
            <div class="chip-dropdown-content">
                <span class="chip-dropdown-text">
                    ${(0, i18n_1.tBi)('Recommended: use a single IDE window. Multi-window setups may cause data desync between instances (e.g. activity timeline, quota tracking).', '建议使用单窗口运行。多窗口可能导致实例间数据不同步（如活动时间线、额度追踪等）。')}
                </span>
            </div>
        </div>
        ...
        <button class="tab-btn active" data-tab="monitor" data-color="blue">${webview_icons_1.ICON.chart} ${(0, i18n_1.tBi)('Monitor', '监控')}</button>
        <button class="tab-btn" data-tab="gmdata" data-color="orange">${webview_icons_1.ICON.bolt} ${(0, i18n_1.tBi)('GM Data', 'GM 数据')}</button>
        <button class="tab-btn" data-tab="chats" data-color="cyan">${webview_icons_1.ICON.chat} ${(0, i18n_1.tBi)('Sessions', '会话')}</button>
        <button class="tab-btn" data-tab="pricing" data-color="purple"><svg class="icon" viewBox="0 0 16 16"><path fill="currentColor" d="M4 10.781c.148 1.667 1.513 2.85 3.591 3.003V15h1.043v-1.216c2.27-.179 3.678-1.438 3.678-3.315 0-1.667-1.104-2.512-3.233-3.037l-.445-.107V3.63c1.213.183 1.968.91 2.141 1.88h1.762c-.112-1.796-1.519-2.965-3.455-3.124V1.036H8.59v1.383C6.408 2.583 5.008 3.9 5.003 5.54c0 1.592 1.063 2.457 3.146 2.963l.399.1v3.979c-1.29-.183-2.113-.879-2.275-1.8H4zm4.586-4.34C7.494 6.137 6.94 5.695 6.94 5.092c0-.66.52-1.183 1.575-1.37v2.72h.071zm.889 2.283c1.335.36 1.942.846 1.942 1.548 0 .781-.633 1.35-1.823 1.493V8.851l-.119-.127z"/></svg> ${(0, i18n_1.tBi)('Cost', '成本')}</button>
        <button class="tab-btn" data-tab="models" data-color="green">${webview_icons_1.ICON.bolt} ${(0, i18n_1.tBi)('Models', '模型')}</button>
        <button class="tab-btn" data-tab="history" data-color="yellow">${webview_icons_1.ICON.timeline} ${(0, i18n_1.tBi)('Quota Tracking', '额度追踪')}</button>
        <button class="tab-btn" data-tab="calendar" data-color="cyan"><svg class="icon" viewBox="0 0 16 16"><path fill="currentColor" d="M3.5 0a.5.5 0 0 1 .5.5V1h8V.5a.5.5 0 0 1 1 0V1h1a2 2 0 0 1 2 2v11a2 2 0 0 1-2 2H2a2 2 0 0 1-2-2V3a2 2 0 0 1 2-2h1V.5a.5.5 0 0 1 .5-.5M1 4v10a1 1 0 0 0 1 1h12a1 1 0 0 0 1-1V4z"/></svg> ${(0, i18n_1.tBi)('Calendar', '日历')}</button>
        <button class="tab-btn" data-tab="profile" data-color="gray">${webview_icons_1.ICON.user} ${(0, i18n_1.tBi)('Profile', '个人')}</button>
        <button class="tab-btn" data-tab="settings" data-color="gray">${webview_icons_1.ICON.shield} ${(0, i18n_1.tBi)('Settings', '设置')}</button>
    </nav>
        <button class="tab-arrow tab-arrow-right is-faded" id="tabArrowRight" aria-label="${(0, i18n_1.tBi)('Scroll tabs right', '向右滚动标签')}">
    ...
    <div class="tab-scroll-hint" id="tabScrollHint" hidden>
        <span class="tab-scroll-hint-text">${webview_icons_1.ICON.timeline} <span>${(0, i18n_1.tBi)('Too many tabs? Hold Shift and use the mouse wheel to scroll horizontally.', '标签过多时，可按住 Shift 再滚动鼠标滚轮进行横向滚动。')}</span></span>
        <button class="tab-scroll-hint-close" id="dismissTabScrollHint" aria-label="${(0, i18n_1.tBi)('Dismiss tab scroll hint', '关闭标签滚动提示')}">
    ...
    <div class="tab-pane active" id="tab-monitor">
        ${monitorHtml}
        <div class="eoc-sentinel"><span class="eoc-sentinel-text">${(0, i18n_1.tBi)('— End of content —', '— 已到底 —')}</span></div>
    </div>
