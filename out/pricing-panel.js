...
exports.buildPricingTabContent = buildPricingTabContent;
exports.getPricingTabStyles = getPricingTabStyles;
exports.buildModelDNACards = buildModelDNACards;
...
// ─── Public API ──────────────────────────────────────────────────────────────
function buildPricingTabContent(summary, store, monthBreakdown) {
    ...
    if (hasGM && costResult) {
        ...
        parts.push(buildCostVisualization(rows, grandTotal, summary), buildCostSummary(rows, grandTotal), buildEditablePricingTable(summary, merged, store.getCustom()));
    }
    else {
        // No GM data yet — still show the editable pricing table with defaults
        parts.push(`<p class="empty-msg">${(0, i18n_1.tBi)('Cost analysis will appear after GM data is available. You can configure custom prices below.', '费用分析将在 GM 数据可用后显示。您可以在下方配置自定义价格。')}</p>`, buildDefaultPricingTable(store.getMerged(), store.getCustom()));
    }
    return parts.join('');
}
function getPricingTabStyles() {
    return `
    /* ── Model DNA Cards ── */
    .prc-dna-grid {
        display: grid;
...
// ─── Cost Visualization ──────────────────────────────────────────────────────
function buildCostVisualization(rows, grandTotal, summary) {
    const priced = rows.filter(r => r.pricing && r.totalCost > 0);
    if (priced.length === 0 || grandTotal <= 0) {
        return '';
    }
    const fmtUsd = (n) => n < 0.01 ? `$${n.toFixed(4)}` : n < 1 ? `$${n.toFixed(3)}` : `$${n.toFixed(2)}`;
    // Highlights
    const topModel = priced[0];
    const avgPerCall = summary.totalCalls > 0 ? grandTotal / summary.totalCalls : 0;
    let html = `<h2 class="act-section-title">${(0, i18n_1.tBi)('Cost Overview', '费用概览')} <span class="gm-badge-real">${(0, i18n_1.tBi)('Visual', '可视化')}</span></h2>`;
    ...
    html += `<div class="prc-viz-highlight">
        ...
        <div class="prc-viz-hl-label">${(0, i18n_1.tBi)('Total Cost', '总费用')}</div>
    </div>`;
    html += `<div class="prc-viz-highlight">
        <div class="prc-viz-hl-val" style="color:#2dd4bf">${(0, webview_helpers_1.esc)(topModel.name)}</div>
        <div class="prc-viz-hl-label">${(0, i18n_1.tBi)('Top Spender', '最高消费')}</div>
    </div>`;
    html += `<div class="prc-viz-highlight">
        <div class="prc-viz-hl-val" style="color:#60a5fa">${fmtUsd(avgPerCall)}</div>
        <div class="prc-viz-hl-label">${(0, i18n_1.tBi)('Avg/Call', '平均/次')}</div>
    </div>`;
    html += `<div class="prc-viz-highlight">
        <div class="prc-viz-hl-val">${priced.length}</div>
        <div class="prc-viz-hl-label">${(0, i18n_1.tBi)('Models', '模型数')}</div>
    </div>`;
    html += '</div>';
    // Bar chart
    ...
    for (const r of priced) {
        ...
        html += `<div class="prc-bar-row">
            <span class="prc-bar-label">${(0, webview_helpers_1.esc)(r.name)}</span>
            <div class="prc-bar-track">
                ${inputPct > 0 ? `<div class="prc-bar-seg prc-bar-seg-input" style="width:${inputPct.toFixed(1)}%" data-tooltip="${(0, i18n_1.tBi)('Input', '输入')}: ${fmtUsd(r.inputCost)}"></div>` : ''}
                ${outputPct > 0 ? `<div class="prc-bar-seg prc-bar-seg-output" style="width:${outputPct.toFixed(1)}%" data-tooltip="${(0, i18n_1.tBi)('Output', '输出')}: ${fmtUsd(r.outputCost)}"></div>` : ''}
                ${cachePct > 0 ? `<div class="prc-bar-seg prc-bar-seg-cache" style="width:${cachePct.toFixed(1)}%" data-tooltip="${(0, i18n_1.tBi)('Cache', '缓存')}: ${fmtUsd(r.cacheCost + r.cacheWriteCost)}"></div>` : ''}
                ${thinkPct > 0 ? `<div class="prc-bar-seg prc-bar-seg-thinking" style="width:${thinkPct.toFixed(1)}%" data-tooltip="${(0, i18n_1.tBi)('Thinking', '思考')}: ${fmtUsd(r.thinkingCost)}"></div>` : ''}
            </div>
    ...
    html += `<div class="prc-bar-legend">
        <span class="prc-bar-legend-item"><span class="prc-bar-legend-dot" style="background:#60a5fa"></span> ${(0, i18n_1.tBi)('Input', '输入')}</span>
        <span class="prc-bar-legend-item"><span class="prc-bar-legend-dot" style="background:#2dd4bf"></span> ${(0, i18n_1.tBi)('Output', '输出')}</span>
        <span class="prc-bar-legend-item"><span class="prc-bar-legend-dot" style="background:#22d3ee"></span> ${(0, i18n_1.tBi)('Cache', '缓存')}</span>
        <span class="prc-bar-legend-item"><span class="prc-bar-legend-dot" style="background:#fb923c"></span> ${(0, i18n_1.tBi)('Thinking', '思考')}</span>
    </div>`;
    html += '</div>';
    return html;
}
// ─── Section Builders ────────────────────────────────────────────────────────
function buildModelDNACards(s, persisted = {}, configs = []) {
    const currentEntries = Object.entries(s?.modelBreakdown || {});
    const currentByKey = new Map();
    for (const [name, stats] of currentEntries) {
        currentByKey.set((0, model_dna_store_1.getModelDNAKey)(name, stats.responseModel), [name, stats]);
    }
    ...
    for (const config of configs) {
        const normalizedLabel = (0, models_1.normalizeModelDisplayName)(config.label) || config.label;
        configByLabel.set(normalizedLabel, config);
    }
    let html = `<h2 class="act-section-title">${(0, i18n_1.tBi)('Model Info', '模型信息')}</h2>`;
    ...
    for (const entry of entries) {
        ...
        const config = configByLabel.get((0, models_1.normalizeModelDisplayName)(name) || name);
        ...
        const mimeDetailsHtml = supportedMimeTypes.length > 0
            ? `
                <details class="collapsible inline-details" id="d-model-mime-${entryId}">
                    <summary>${(0, i18n_1.tBi)('MIME Types', 'MIME 类型')} (${supportedMimeTypes.length})</summary>
                    <div class="details-body">
                        <div class="mime-tags-wrap">
                            ${supportedMimeTypes.map(mime => `<span class="mime-tag">${(0, webview_helpers_1.esc)(mime)}</span>`).join('')}
                        </div>
                    </div>
                </details>`
            : '';
        const techDetailsHtml = cc
            ? `
                <details class="collapsible inline-details" id="d-model-tech-${entryId}">
                    <summary>${(0, i18n_1.tBi)('Technical Params', '技术参数')}</summary>
                    <div class="details-body">
                        <div class="prc-dna-grid-inner">
                            ${buildDNAField('maxTokens', String(cc.maxTokens))}
                            ${buildDNAField((0, i18n_1.tBi)('temp', '温度'), cc.temperature.toString())}
                            ${buildDNAField((0, i18n_1.tBi)('firstTemp', '初始温度'), cc.firstTemperature.toString())}
                            ${buildDNAField('topK', String(cc.topK))}
                            ${buildDNAField('topP', cc.topP.toString())}
                            ${buildDNAField((0, i18n_1.tBi)('stops', '停止词'), String(cc.stopPatternCount))}
                        </div>
                    </div>
                </details>`
            : '';
        ...
        html += `<div class="prc-dna-header">
            <span class="prc-dna-model">${(0, webview_helpers_1.esc)(name)}</span>
        </div>`;
        const metaParts = [];
        if (responseModel) {
            metaParts.push(`<span class="prc-dna-response-model">${(0, webview_helpers_1.esc)(responseModel)}</span>`);
        }
        if (providerShort) {
            metaParts.push(`<span class="prc-dna-provider">${(0, webview_helpers_1.esc)(providerShort)}</span>`);
        }
        if (isPersistedOnly) {
            metaParts.push(`<span class="prc-dna-provider">${(0, i18n_1.tBi)('cached', '已缓存')}</span>`);
        }
        if (metaParts.length > 0) {
            html += `<div class="prc-dna-meta">${metaParts.join('<span class="prc-dna-sep">·</span>')}</div>`;
        }
        html += `<div class="prc-dna-grid-inner">`;
        html += buildDNAField((0, i18n_1.tBi)('Calls', '调用'), String(callCount));
        html += buildDNAField((0, i18n_1.tBi)('Steps', '步骤'), String(stepsCovered));
        html += buildDNAField((0, i18n_1.tBi)('Credits', '积分'), String(totalCredits));
        if (totalRetries > 0) {
            html += buildDNAField((0, i18n_1.tBi)('Retries', '重试'), String(totalRetries));
        }
        if (errorCount > 0) {
            html += `<div class="prc-dna-field"><span class="prc-dna-label">${(0, i18n_1.tBi)('Errors', '错误')}</span><span class="prc-dna-val" style="color:#ef4444">${errorCount}</span></div>`;
        }
        html += `</div>`;
...
function buildDNAField(label, value) {
    return `<div class="prc-dna-field"><span class="prc-dna-label">${(0, webview_helpers_1.esc)(label)}</span><span class="prc-dna-val">${(0, webview_helpers_1.esc)(value)}</span></div>`;
}
function toDomSafeId(value) {
    return value.replace(/[^a-zA-Z0-9_-]+/g, '-');
}
// ─── Cost Summary (Card-based) ───────────────────────────────────────────────
function buildCostSummary(rows, grandTotal) {
    if (rows.length === 0) {
        return '';
    }
    const fmtUsd = (n) => n < 0.01 ? `$${n.toFixed(4)}` : n < 1 ? `$${n.toFixed(3)}` : `$${n.toFixed(2)}`;
    const fmt = (n) => n >= 1_000_000 ? (n / 1_000_000).toFixed(2) + 'M' : n >= 1000 ? (n / 1000).toFixed(1) + 'k' : String(n);
    let html = `<h2 class="act-section-title">${(0, i18n_1.tBi)('Cost Breakdown', '费用明细')} <span class="gm-badge-real">${(0, i18n_1.tBi)('Per Model', '按模型')}</span></h2>`;
    // Grand total card
    html += `<div class="prc-cost-grid">`;
    html += `<div class="prc-cost-card prc-cost-grand">
        <div class="prc-cost-card-header">
            <span class="prc-cost-card-name">${(0, i18n_1.tBi)('Grand Total', '总计')}</span>
        </div>
        <div class="prc-cost-grand-val">${fmtUsd(grandTotal)}</div>
        <div class="prc-note" style="margin-top:var(--space-1)">${rows.filter(r => r.pricing).length} ${(0, i18n_1.tBi)('models priced', '个模型有定价')}</div>
    </div>`;
    // Per-model cards
    for (const r of rows) {
        if (!r.pricing) {
            html += `<div class="prc-cost-card">
                <div class="prc-cost-card-header">
                    <span class="prc-cost-card-name" data-tooltip="${(0, webview_helpers_1.esc)(r.responseModel)}">${(0, webview_helpers_1.esc)(r.name)}</span>
                </div>
                <div class="prc-cost-no-pricing">${(0, i18n_1.tBi)('No pricing data available', '暂无价格数据')}</div>
            </div>`;
            continue;
        }
        html += `<div class="prc-cost-card">
            ...
            <div class="prc-cost-card-body">
                <div class="prc-cost-item" data-tooltip="${(0, i18n_1.tBi)(`${fmt(r.inputTokens)} tok × $${r.pricing.input}/M`, `${fmt(r.inputTokens)} 令牌 × $${r.pricing.input}/百万`)}">
                    <span class="prc-cost-item-label"><span class="prc-cost-item-dot" style="background:#60a5fa"></span>${(0, i18n_1.tBi)('Input', '输入')}</span>
                    <span class="prc-cost-item-val">${fmtUsd(r.inputCost)}</span>
                </div>
                <div class="prc-cost-item" data-tooltip="${(0, i18n_1.tBi)(`${fmt(r.outputTokens)} tok × $${r.pricing.output}/M`, `${fmt(r.outputTokens)} 令牌 × $${r.pricing.output}/百万`)}">
                    <span class="prc-cost-item-label"><span class="prc-cost-item-dot" style="background:#2dd4bf"></span>${(0, i18n_1.tBi)('Output', '输出')}</span>
                    <span class="prc-cost-item-val">${fmtUsd(r.outputCost)}</span>
                </div>
                <div class="prc-cost-item" data-tooltip="${(0, i18n_1.tBi)(`${fmt(r.cacheTokens)} tok × $${r.pricing.cacheRead}/M`, `${fmt(r.cacheTokens)} 令牌 × $${r.pricing.cacheRead}/百万`)}">
                    <span class="prc-cost-item-label"><span class="prc-cost-item-dot" style="background:#22d3ee"></span>${(0, i18n_1.tBi)('Cache Read', '缓存读取')}</span>
                    <span class="prc-cost-item-val">${fmtUsd(r.cacheCost)}</span>
                </div>
                <div class="prc-cost-item" data-tooltip="${(0, i18n_1.tBi)(`${fmt(r.cacheWriteTokens)} tok × $${r.pricing.cacheWrite}/M`, `${fmt(r.cacheWriteTokens)} 令牌 × $${r.pricing.cacheWrite}/百万`)}">
                    <span class="prc-cost-item-label"><span class="prc-cost-item-dot" style="background:#22d3ee"></span>${(0, i18n_1.tBi)('Cache Write', '缓存写入')}</span>
                    <span class="prc-cost-item-val">${fmtUsd(r.cacheWriteCost)}</span>
                </div>
                ${r.thinkingTokens > 0 ? `<div class="prc-cost-item" data-tooltip="${(0, i18n_1.tBi)(`${fmt(r.thinkingTokens)} tok × $${r.pricing.thinking}/M`, `${fmt(r.thinkingTokens)} 令牌 × $${r.pricing.thinking}/百万`)}">
                    <span class="prc-cost-item-label"><span class="prc-cost-item-dot" style="background:#fb923c"></span>${(0, i18n_1.tBi)('Thinking', '思考')}</span>
    ...
    html += `</div>`;
    html += `<p class="prc-note">${(0, i18n_1.tBi)('Costs are estimates based on the pricing table below. Actual billing may differ with enterprise agreements.', '费用基于下方价格表估算。实际计费可能因企业协议而不同。')}</p>`;
    return html;
}
...
function buildEditablePricingTable(summary, merged, custom) {
    const entries = Object.entries(summary.modelBreakdown);
    if (entries.length === 0) {
        return '';
    }
    const fields = ['input', 'output', 'cacheRead', 'cacheWrite', 'thinking'];
    let html = `<h2 class="act-section-title">${(0, i18n_1.tBi)('Custom Pricing', '自定义价格')} <span style="font-size:0.82em;color:var(--color-text-dim)">(${(0, i18n_1.tBi)('USD / 1M tokens', 'USD / 100万令牌')})</span></h2>`;
    html += `<div class="prc-edit-section">`;
    html += `<div class="prc-edit-grid">`;
    for (const [name, ms] of entries) {
        const pricing = (0, pricing_store_1.findPricing)(ms.responseModel, merged);
        ...
        const sourceText = isCustom ? (0, i18n_1.tBi)('Custom', '自定义') : pricing ? (0, i18n_1.tBi)('Built-in', '内置') : (0, i18n_1.tBi)('None', '无');
        html += `<div class="prc-edit-card">`;
        html += `<div class="prc-edit-card-header">
            <span class="prc-edit-card-name">${(0, webview_helpers_1.esc)(name)}${isCustom ? `<span class="prc-custom-badge">${(0, i18n_1.tBi)('CUSTOM', '自定义')}</span>` : ''}</span>
        ...
        for (const f of fields) {
            ...
            html += `<div class="prc-edit-field">
                <span class="prc-edit-field-label">${(0, i18n_1.tBi)(en, zh)}</span>
                <input type="number" class="prc-edit-input pricing-input" data-model="${(0, webview_helpers_1.esc)(ms.responseModel)}" data-field="${f}" value="${p[f]}" step="0.01" min="0">
            </div>`;
        }
        html += `</div></div>`;
    }
    ...
    html += `<div class="prc-edit-actions">
        <button class="prc-btn prc-btn-primary" id="pricingSaveBtn">${(0, i18n_1.tBi)('Save Prices', '保存价格')}</button>
        <button class="prc-btn" id="pricingResetBtn">${(0, i18n_1.tBi)('Reset to Default', '恢复默认')}</button>
        <span class="prc-feedback" id="pricingFeedback"></span>
    </div>`;
    html += `<p class="prc-note">${(0, i18n_1.tBi)(`Edit prices above and click Save. Changes are persisted across sessions. Reset restores built-in defaults. Default prices last updated: ${pricing_store_1.PRICING_LAST_UPDATED}.`, `编辑上方价格后点击保存。修改跨会话持久化。重置恢复内置默认值。默认价格最后更新：${pricing_store_1.PRICING_LAST_UPDATED}。`)}</p>`;
    html += `</div>`;
    return html;
}
/** Pricing table using DEFAULT_PRICING keys — shown when no GM data is available. */
function buildDefaultPricingTable(merged, custom) {
    ...
    for (const [model, p] of entries) {
        ...
        const sourceText = isCustom ? (0, i18n_1.tBi)('Custom', '自定义') : (0, i18n_1.tBi)('Built-in', '内置');
        html += `<div class="prc-edit-card">`;
        html += `<div class="prc-edit-card-header">
            <span class="prc-edit-card-name">${(0, webview_helpers_1.esc)(model)}${isCustom ? `<span class="prc-custom-badge">${(0, i18n_1.tBi)('CUSTOM', '自定义')}</span>` : ''}</span>
            <span class="prc-edit-source ${sourceClass}">${sourceText}</span>
        </div>`;
        html += `<div class="prc-edit-fields">`;
        for (const f of fields) {
            ...
            html += `<div class="prc-edit-field">
                <span class="prc-edit-field-label">${(0, i18n_1.tBi)(en, zh)}</span>
                <input type="number" class="prc-edit-input pricing-input" data-model="${(0, webview_helpers_1.esc)(model)}" data-field="${f}" value="${p[f]}" step="0.01" min="0">
            </div>`;
        }
        html += `</div></div>`;
    }
    html += `</div>`;
    html += `<div class="prc-edit-actions"><button class="prc-btn prc-btn-primary" id="pricingSaveBtn">${(0, i18n_1.tBi)('Save Prices', '保存价格')}</button><button class="prc-btn" id="pricingResetBtn">${(0, i18n_1.tBi)('Reset to Default', '恢复默认')}</button><span class="prc-feedback" id="pricingFeedback"></span></div>`;
    html += `<p class="prc-note">${(0, i18n_1.tBi)(`Edit prices above and click Save. Changes are persisted across sessions. Default prices last updated: ${pricing_store_1.PRICING_LAST_UPDATED}.`, `编辑上方价格后点击保存。修改跨会话持久化。默认价格最后更新：${pricing_store_1.PRICING_LAST_UPDATED}。`)}</p></div>`;
    return html;
}
...
function buildMonthlyCostSummary(breakdown, currentCycleCost, currentCycleRows) {
    ...
    if (breakdown.earliestDate && isCurrentMonth) {
        ...
        if (dayNum > 1) {
            dataCoverageNote = (0, i18n_1.tBi)(`Data recorded from ${breakdown.earliestDate}. Earlier usage in this month is not tracked.`, `数据从 ${breakdown.earliestDate} 开始记录。本月更早的用量未被追踪。`);
        }
    }
    let html = `<section class="card prc-monthly-section">`;
    // Header with title and calendar link
    html += `<div class="prc-monthly-header">
        <h2>${DOLLAR_ICON} ${(0, i18n_1.tBi)(`${monthEn} ${breakdown.year} Cost`, `${breakdown.year}年${monthZh}费用`)}</h2>
        <button class="prc-monthly-calendar-link" data-switch-tab="calendar">
            ${CALENDAR_LINK_ICON} ${(0, i18n_1.tBi)('View History', '查看历史')}
        </button>
    </div>`;
    if (models.length === 0 && grandTotal === 0) {
        html += `<p class="prc-monthly-empty">${(0, i18n_1.tBi)('No cost data recorded for this month yet.', '本月暂无费用数据。')}</p>`;
        html += `</section>`;
        return html;
    }
    // Grand total highlight
    const archivedLabel = isCurrentMonth
        ? (0, i18n_1.tBi)(`${breakdown.cycleCount} archived cycle${breakdown.cycleCount !== 1 ? 's' : ''} + current`, `${breakdown.cycleCount} 个已归档周期 + 当前`)
        : (0, i18n_1.tBi)(`${totalCycles} cycle${totalCycles !== 1 ? 's' : ''}`, `${totalCycles} 个周期`);
    html += `<div class="prc-monthly-grand">
        <span class="prc-monthly-grand-val">${fmtCost(grandTotal)}</span>
        <span class="prc-monthly-grand-label">${(0, i18n_1.tBi)('Total', '总计')}</span>
    ...
    for (const m of models) {
        ...
        html += `<div class="prc-monthly-card">
            <span class="prc-monthly-model-name">${(0, webview_helpers_1.esc)(m.name)}</span>
        ...
        <div class="prc-monthly-chips" style="padding-left:var(--space-3); margin-top:-4px; margin-bottom:var(--space-1)">
            ...
            <span class="prc-monthly-chip">${m.calls} ${(0, i18n_1.tBi)('calls', '调用')}</span>
        </div>`;
    }
    html += `</div>`;
    // Data coverage note
...
//# sourceMappingURL=pricing-panel.js.map